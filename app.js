...
var reqUserInfo =
    {'userEmail': undefined, 'userPass': undefined, 'authToken': undefined}
...
var startApp =
  ...
  app.use(express.static(
      path.join(__dirname, '/public')));  // Point to Angular frontend content
  ...
  */
  ...
  app.post('/auth/create', function(req, res) {
    ...
    var callbackFunction =
        registerCallBack(handleResponsePassthrough, req, res);
  ...
  app.get('/regions/details', extractUserInfo, function(req, res) {
    ...
    var callbackFunction =
        registerCallBack(handleGetRegionsDetailsResponse, req, res);
  ...
  function handleGetRegionsResponse(responseRegions, httpReq, httpRes) {
    ...
    if (responseRegions.code == 200) {
      ...
      sebalApi.getRegionsDetails(
          reqUserInfo, regionsNames, function(regionDetailsResponse) {
            if (Array.isArray(regionDetailsResponse.data)) {
              regionDetailsResponse.data.forEach(function(regionDetail, index) {
                ...
                regionDetail.processedImages.forEach(function(
                    processedImage, ind) {
  ...
  function extractUserInfo(req, res, next) {
    ...
    if (!reqUserInfo.userEmail || !reqUserInfo.userPass) {
