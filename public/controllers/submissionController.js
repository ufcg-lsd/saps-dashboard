...
dashboardControllers.controller(
    ...
    function(
        $scope, $rootScope, $log, $filter, $timeout, $filter, SubmissionService,
        AuthenticationService, GlobalMsgService, EmailService, appConfig,
        NgTableParams) {
      $scope.ongoingTasks = [];
      ...
      // Script options
      $scope.processingScripts = [{name: 'DEFAULT', value: 'default_script'}]
      ...
      $scope.changePreProcScript =
          function(newScriptOpt) {
        ...
        this.ongoingTasks.forEach(function(item) {
          if (item.name.includes(str) || item.imageDate.includes(str) ||
        ...
        self.ongoingTasks =
            new NgTableParams({count: 4}, {dataset: newOngoing, counts: []});
        self.completedTasksCount = newCompleted.length;
        self.completedTasks =
            new NgTableParams({count: 4}, {dataset: newCompleted, counts: []});
      }
        ...
        submissions.forEach(function(submission, index) {
              ...
              tagParts.forEach(function(tagPart, z) {
                ...
                tagFilterParts.forEach(function(filterTag, y) {
                  if (tagPart.toLowerCase().includes(filterTag.toLowerCase())) {
                    console.log(
                        'Show submission ' + submission.name +
                        ' due tag filter')
                    filterTagsOk = true;
                  }
                })
          ...
          if ($rootScope.validateString(
                  $scope.searchFilters.processingScript)) {
            if (submission.processingScript ==
                $scope.searchFilters.processingScript) {
              console.log(
                  'Show submission ' + submission.name +
                  ' due procScript filter')
          ...
          if ($rootScope.validateString(
            ...
            if (submission.preProcessingScript ==
                $scope.searchFilters.preProcessingScript) {
              console.log(
                  'Show submission ' + submission.name +
                  ' due preProcScript filter')
          ...
          if ($rootScope.validateDate($('#filter-initial-date-input').val())) {
            initialDate =
                $rootScope.parseDate($('#filter-initial-date-input').val())
          }

          if ($rootScope.validateDate($('#filter-final-date-input').val())) {
            finalDate =
                $rootScope.parseDate($('#filter-final-date-input').val())
          }
      ...
      function isCompleted(processing) {
        return 'state' in processing &&
            (processing.state.toUpperCase() === 'SUCCESS' ||
             processing.state.toUpperCase() === 'FAILURE');
      }
      ...
      var updateProcessingsByState =
        ...
        self.ongoingTasks = new NgTableParams(
            {count: 4}, {dataset: $scope.ongoingTasks, counts: []});
        self.completedTasksCount = $scope.completedTasks.length;
        self.completedTasks = new NgTableParams(
            {count: 4}, {dataset: $scope.completedTasks, counts: []});
      }

          $scope.generateTagsComponent =
              function(submission) {
...
dashboardControllers.controller(
    ...
    function(
        $scope, $rootScope, $log, $filter, $timeout, AuthenticationService,
        SubmissionService, GlobalMsgService, appConfig) {
      ...
      $scope.cleanForm =
        ...
        if ($scope.newSubmission.lowerLeftCoord.lat === undefined ||
          ...
          if (isNaN($scope.newSubmission.lowerLeftCoord.lat) ||
        ...
        if ($scope.newSubmission.initialDate === undefined) {
          ...
          data.initialDate =
              $scope.newSubmission.initialDate.toISOString().slice(0, 11);
        }
        if ($scope.newSubmission.finalDate === undefined) {
          ...
          data.finalDate =
              $scope.newSubmission.finalDate.toISOString().slice(0, 11);
        }
        ...
        if ($scope.newSubmission.priority < 1 ||
        ...
        data.inputPreprocessingTag =
            $scope.newSubmission.inputPreprocessing.name;
        data.algorithmExecutionTag =
            $scope.newSubmission.algorithmExecution.name;
        ...
        // $rootScope.languageContent.mapFeature.submissionBox.label.loadSubmission;
        // $scope.openCloseModal('loadingModal', true);

        SubmissionService.postSubmission(
            data,
            function() {
              // GlobalMsgService.pushMessageSuccess('Your job was submitted.
              // Wait for the processing be completed. '
              //       + 'If you activated the notifications you will get an
              //       email when finished.');
