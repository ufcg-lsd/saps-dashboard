 ...
 */
var CLOSE_EVENT = 'Close', BEFORE_CLOSE_EVENT = 'BeforeClose',
    AFTER_CLOSE_EVENT = 'AfterClose', BEFORE_APPEND_EVENT = 'BeforeAppend',
    MARKUP_PARSE_EVENT = 'MarkupParse', OPEN_EVENT = 'Open',
    CHANGE_EVENT = 'Change', NS = 'mfp', EVENT_NS = '.' + NS,
    READY_CLASS = 'mfp-ready', REMOVING_CLASS = 'mfp-removing',
...
var mfp,  // As we have only one instance of MagnificPopup object, we define it
    ...
    MagnificPopup = function() {}, _isJQ = !!(window.jQuery), _prevStatus,
    _window = $(window), _body, _document, _prevContentType, _wrapClasses,
    _currPopupType;
...
var _mfpOn =
  ...
  mfp.ev.on(NS + name + EVENT_NS, f);
    ...
    _getEl =
        function(className, appendTo, html, raw) {
    ...
    supportsTransitions = function() {
      var s = document.createElement('p')
                  .style,  // 's' for style. better to create an element if body
                           // yet to exist
...
MagnificPopup.prototype = {
  ...
  _openClick: function(e, el, options) {
    ...
    if (e.type) {
      ...
      // This will prevent popup from closing if element is inside and popup is
      // already opened
      if (mfp.isOpen) {
        e.stopPropagation();
      }
      ...
      var opts = $.extend(
          {
            ...
            error: function() {
              ...
              mfp.updateStatus(
                  'error', mfp.st.ajax.tError.replace('%url%', item.src));
            }
          },
...
$.magnificPopup.registerModule('image', {
  ...
  proto: {
    ...
    getImage: function(item, template) {
          ...
          onLoadError = function() {
            if (item) {
              ...
              if (item === mfp.currItem) {
                mfp._onImageHasSize(item);
                mfp.updateStatus(
                    'error', imgSt.tError.replace('%url%', item.src));
              }
      ...
      mfp._parseMarkup(
          template, {title: _getTitle(item), img_replaceWith: item.img}, item);
  ...
  proto: {
    ...
    initZoom: function() {
      ...
      var duration = zoomSt.duration, getElToAnimate = function(image) {
        var newImg =
                image.clone().removeAttr('style').removeAttr('class').addClass(
                    'mfp-animated-image'),
            transition =
        ...
        cssObj['-webkit-' + t] = cssObj['-moz-' + t] = cssObj['-o-' + t] =
            cssObj[t] = transition;
    ...
    _getOffset: function(isLarge) {
      ...
      Animating left + top + width/height looks glitchy in Firefox, but perfect
    ...
    initGallery: function() {
      ...
      _mfpOn('UpdateStatus' + ns, function(e, data) {
        if (data.text) {
          data.text = _replaceCurrTotal(
              data.text, mfp.currItem.index, mfp.items.length);
        }
      });
      ...
      _mfpOn('BuildControls' + ns, function() {
        if (mfp.items.length > 1 && gSt.arrows && !mfp.arrowLeft) {
          ...
          // Polyfill for :before and :after (adds elements with classes mfp-a
          // and mfp-b)
          if (mfp.isIE7) {
 ...
 *
 *
 * Note that it's a very basic and simple implementation, it blocks ghost click
 * on the same element where it was bound. If you need something more advanced,
 * use plugin by FT Labs https://github.com/ftlabs/fastclick
 *
 */

(function() {
var ghostClickDelay = 1000, supportsTouch = 'ontouchstart' in window,
    unbindTouchMove = function() {
      _window.off('touchmove' + ns + ' touchend' + ns);
    }, eName = 'mfpFastClick', ns = '.' + eName;


// As Zepto.js doesn't have an easy way to add custom events (like jQuery), so
// we implement it in this way
