  ...
  jQuery.sessionTimeout = function(b) {
    ...
    function e() {
      clearTimeout(g),
          ...
          !i.countdownMessage && !i.countdownBar || f('dialog', !0),
          g = setTimeout(function() {
            'function' != typeof i.onRedir ? window.location = i.redirUrl :
                                             i.onRedir(i)
    ...
    function f(b, c) {
      clearTimeout(j.timer),
          ...
          i.countdownBar && 'session' === b &&
              (j.percentLeft =
                   Math.floor(j.timeLeft / (i.redirAfter / 1e3) * 100)),
          a('.countdown-holder').text(j.timeLeft),
          i.countdownBar &&
          a('.countdown-bar').css('width', j.percentLeft + '%'),
          j.timeLeft = j.timeLeft - 1, j.timer = setTimeout(function() {
                                         f(b)
                                       }, 1e3)
    }
    var g, h = {
    ...
    if ('function' != typeof i.onWarn) {
      var k = i.countdownMessage ? '<p>' +
              i.countdownMessage.replace(
                  /{timer}/g, '<span class="countdown-holder"></span>') +
              '</p>' :
                                   '',
          l = i.countdownBar ?
          '<div class="progress">                   <div class="progress-bar progress-bar-striped countdown-bar active" role="progressbar" style="min-width: 15px; width: 100%;">                     <span class="countdown-holder"></span><span>s</span>                   </div>                 </div>' :
          '';
      a('body').append(
    ...
    i.ignoreUserActivity ||
        a(document).on(
            'keyup mouseup mousemove touchend touchmove', function() {
              d()
            });
    var m = !1;
    d()
