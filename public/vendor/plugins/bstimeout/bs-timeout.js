...
jQuery.sessionTimeout = function(options) {
  ...
  // Unless user set his own callback function, prepare bootstrap modal elements
  // and events
  if (typeof opt.onWarn !== 'function') {
    // If opt.countdownMessage is defined add a coundown timer message to the
    // modal dialog
    var countdownMessage = opt.countdownMessage ? '<p>' +
            opt.countdownMessage.replace(
                /{timer}/g, '<span class="countdown-holder"></span>') +
            '</p>' :
                                                  '';
  ...
  // Keeps the server side connection live, by pingin url set in keepAliveUrl
  // option. KeepAlivePinged is a helper var to ensure the functionality of the
  ...
  function keepAlive() {
    if (!keepAlivePinged) {
      ...
      $.ajax({type: opt.ajaxType, url: opt.keepAliveUrl, data: opt.ajaxData});
  ...
  function startDialogTimer() {
    ...
    // Set dialog timer
    timer = setTimeout(function() {
      // Check for onRedir callback function and if there is none, launch
      // redirect
  ...
  function startCountdownTimer(type, reset) {
    ...
    if (opt.countdownBar && type === 'dialog') {
      ...
      countdown.percentLeft =
          Math.floor(countdown.timeLeft / (opt.redirAfter / 1000) * 100);
    }
