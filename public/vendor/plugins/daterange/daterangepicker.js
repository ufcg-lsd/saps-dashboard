...
var DateRangePicker = function(element, options, cb) {
  // by default, the daterangepicker element is placed at the bottom of HTML
  // body
  this.parentEl = 'body';
...
DateRangePicker.prototype = {
  ...
  setOptions: function(options, callback) {
    ...
    // bind the time zone used to build the calendar to either the timeZone
    // passed in through the options or the zone of the startDate (which will be
    ...
    if (typeof options.ranges === 'object') {
      for (range in options.ranges) {
        ...
        // the start of the range is after the max (also if set) don't display
        // this range option.
    ...
    this.leftCalendar = {
      month: moment([
        this.startDate.year(), this.startDate.month(), 1, this.startDate.hour(),
        this.startDate.minute(), this.startDate.second()
      ]),
      calendar: []
    };
    ...
    this.rightCalendar = {
      month: moment([
        this.endDate.year(), this.endDate.month(), 1, this.endDate.hour(),
        this.endDate.minute(), this.endDate.second()
      ]),
      calendar: []
    };
  ...
  updateFromControl: function() {
    ...
    var dateString = this.element.val().split(this.separator), start = null,
        end = null;
    ...
    if (this.opens == 'left') {
      ...
      this.container.css({
        ...
        left: this.element.offset().left - parentOffset.left +
            this.element.outerWidth() / 2 - this.container.outerWidth() / 2,
        right: 'auto'
      });
      if (this.container.offset().left < 0) {
        this.container.css({right: 'auto', left: 9});
  ...
  show: function(e) {
    ...
    // Create a click proxy that is private to this instance of datepicker, for
    // unbinding
    ...
    $(document)
        ...
        // and also close when focus changes to outside the picker (eg. tabbing
        // between controls)
  ...
  updateInputText: function() {
    if (this.element.is('input') && !this.singleDatePicker) {
      this.element.val(
          this.startDate.format(this.format) + this.separator +
  ...
  updateCalendars: function() {
    this.leftCalendar.calendar = this.buildCalendar(
        this.leftCalendar.month.month(), this.leftCalendar.month.year(),
        this.leftCalendar.month.hour(), this.leftCalendar.month.minute(),
        this.leftCalendar.month.second(), 'left');
    this.rightCalendar.calendar = this.buildCalendar(
        this.rightCalendar.month.month(), this.rightCalendar.month.year(),
        this.rightCalendar.month.hour(), this.rightCalendar.month.minute(),
        this.rightCalendar.month.second(), 'right');
    ...
    this.container.find('.calendar.right')
        .empty()
        .html(this.renderCalendar(
            this.rightCalendar.calendar, this.endDate,
            this.singleDatePicker ? this.minDate : this.startDate, this.maxDate,
            'right'));
  ...
  renderDropdowns: function(selected, minDate, maxDate) {
    ...
    for (var y = minYear; y <= maxYear; y++) {
      yearHtml += '<option value="' + y + '"' +
          (y === currentYear ? ' selected="selected"' : '') + '>' + y +
          '</option>';
    }
    ...
    for (var row = 0; row < 6; row++) {
      ...
      for (var col = 0; col < 7; col++) {
        ...
        html += '<td class="' +
            ...
            '" data-title="' + title + '">' + calendar[row][col].date() +
            '</td>';
      }
      html += '</tr>';
    }
    ...
    if (this.timePicker) {
      ...
      for (i = start; i <= end; i++) {
        if (i == selected_hour) {
          ...
          html += '<option value="' + i +
              '" disabled="disabled" class="disabled">' + i + '</option>';
        } else {
      ...
      if (minDate && (side == 'left' || this.singleDatePicker) &&
          selected.format('YYYY-MM-DD h A') ==
              minDate.format('YYYY-MM-DD h A')) {
      ...
      for (i = 0; i < 60; i += this.timePickerIncrement) {
        ...
        if (i == selected.minute()) {
          html += '<option value="' + i + '" selected="selected">' + num +
              '</option>';
        } else if (i < min_minute || i > max_minute) {
          html += '<option value="' + i +
              '" disabled="disabled" class="disabled">' + num + '</option>';
        } else {
