...
CKEDITOR.dialog.add('checkspell', function(a) {
  ...
  var b = CKEDITOR.tools.getNextNumber(), d = 'cke_frame_' + b,
      g = 'cke_data_' + b, h = 'cke_error_' + b, e,
      b = document.location.protocol || 'http:', i = a.lang.wsc.notAvailable,
      k = '<textarea style="display: none" id="' + g +
      '" rows="10" cols="40"> </textarea><div id="' + h +
      ...
      d + '" id="' + d + '" allowtransparency="1"></iframe>',
      l = a.config.wsc_customLoaderScript ||
      b + '//loader.webspellchecker.net/sproxy_fck/sproxy.php?plugin=fck2&customerid=' +
          a.config.wsc_customerId + '&cmd=script&doc=wsc&schema=22';
      ...
      (i +=
       '<p style="color:#000;font-size:11px;font-weight: normal;text-align:center;padding-top:10px">' +
           a.lang.wsc.errorLoading.replace(
               /%s/g, a.config.wsc_customLoaderScript) +
           '</p>');
  ...
  return {
    title: a.config.wsc_dialogTitle || a.lang.wsc.title, minWidth: 485,
        minHeight: 380, buttons: [CKEDITOR.dialog.cancelButton],
        onShow: function() {
          ...
          'function' != typeof window.doSpell &&
              CKEDITOR.document.getHead().append(
                  CKEDITOR.document.createElement(
                      'script',
...
CKEDITOR.dialog.on('resize', function(a) {
  ...
  'checkspell' == c._.name &&
      ((c = (c = c.getContentElement('general', 'content').getElement()) &&
            c.getChild(2)) &&
