...
CKEDITOR.dialog.add('a11yHelp', function(j) {
  ...
  var f = [CKEDITOR.ALT, CKEDITOR.SHIFT, CKEDITOR.CTRL], m = /\$\{(.*?)\}/g,
      p = function() {
  ...
  return {
    title: a.title, minWidth: 600, minHeight: 400, contents: [{
      ...
      elements: [{
        ...
        html: function() {
          for (
              var e = '<div class="cke_accessibility_legend" role="document" aria-labelledby="' +
                  l + '_arialbl" tabIndex="-1">%1</div><span id="' + l +
                  '_arialbl" class="cke_voice_label">' + a.contents +
                  ' </span>',
                  g = [], c = a.legend, j = c.length, f = 0;
              f < j; f++) {
            for (var b = c[f], h = [], i = b.items, k = i.length, d = 0; d < k;
                 d++) {
              var n = i[d], o = n.legend.replace(m, p);
              o.match(m) ||
                  h.push('<dt>%1</dt><dd>%2</dd>'.replace('%1', n.name)
                             .replace('%2', o))
            }
            g.push('<h1>%1</h1><dl>%2</dl>'.replace('%1', b.name)
                       .replace('%2', h.join('')))
          }
