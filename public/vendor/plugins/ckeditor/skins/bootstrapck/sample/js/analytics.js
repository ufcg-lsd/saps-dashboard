...
_ga.trackFacebook = function(a, c) {
  ...
  try {
    FB && FB.Event && FB.Event.subscribe &&
...
_ga.trackTwitter = function(a, c) {
  var d = _ga.buildTrackerName_(c);
  try {
    twttr && twttr.events && twttr.events.bind &&
        twttr.events.bind('tweet', function(b) {
          if (b) {
            var c;
            b.target && 'IFRAME' == b.target.nodeName &&
...
_ga.extractParamFromUri_ = function(a, c) {
  if (a) {
    var a = a.split('#')[0], d = a.split('?');
    if (1 != d.length)
      for (var d = decodeURI(d[1]), c = c + '=', d = d.split('&'), e = 0, b;
           b = d[e]; ++e)
