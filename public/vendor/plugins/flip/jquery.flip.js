 ...
 *
 * @changelog
 * v 0.9.9      ->      Fix transparency over non-colored background. Added
 * dontChangeColor option. Added $clone and $this parameters to on.. callback
 * functions. Force hexadecimal color values. Made safe for noConflict use. Some
 * refactoring. [Henrik Hjelte, Jul. 10, 2009] Added revert options, fixes and
 * improvements on color management. Released in Nov 2009 v 0.5        -> Added
 * patch to make it work with Opera (thanks to Peter Siewert), Added callbacks
 * [Feb. 1, 2008] v 0.4.1      ->      Fixed a regression in Chrome and Safari
 * caused by getTransparent [Oct. 1, 2008] v 0.4        ->      Fixed some bugs
 * with transparent color. Now Flip! works on non-white backgrounds | Update:
 * jquery.color.js plugin or jqueryUI still needed :( [Sept. 29, 2008] v 0.3 ->
 * Now is possibile to define the content after the animation. (jQuery object or
 * text/html is allowed) [Sept. 25, 2008] v 0.2        ->      Fixed
 * chainability and buggy innertext rendering (xNephilimx thanks!) v 0.1 ->
 * Starting release [Sept. 11, 2008]
 *
 */
(function($) {
...
var isIE6orOlder = function() {
  // User agent sniffing is clearly out of fashion and $.browser will be be
  // deprectad. Now, I can't think of a way to feature detect that IE6 doesn't
  // show transparent borders in the correct way. Until then, this function will
  // do, and be partly political correct, allowing 0.01 percent of the internet
  // users to tweak with their UserAgent string.
  //
  // Not leadingWhiteSpace is to separate IE family from, well who knows?
  // Maybe some version of Opera?
  // The second guess behind this is that IE7+  will keep supporting maxHeight
  // in the future.

  // First guess changed to dean edwards ie sniffing
...
$.fn.flip = function(settings) {
  return this.each(function() {
    ...
    $this.data('flipRevertedSettings', revertedSettings)
        .data('flipLock', 1)
    ...
    flipObj = {
      ...
      height: $this.height(),
      bgColor:
          acceptHexColor(settings.bgColor) || $this.css('background-color'),
    ...
    // The color below is arbitrary, lets just hope it is not used in the
    // animation
