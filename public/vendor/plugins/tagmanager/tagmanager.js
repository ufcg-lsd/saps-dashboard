  ...
  fillInputOnTagRemove: false,
    ...
    publicMethods = {
      pushTag: function(tag, ignoreEvents, externalTagId) {
        var $self = $(this), opts = $self.data('opts'), alreadyInList,
            tlisLowerCase, max, tagId, tlis = $self.data('tlis'),
            tlid = $self.data('tlid'), idx, newTagId, newTagRemoveId, escaped,
        ...
        if (opts.onlyTagList && undefined !== opts.tagList) {
          // if the list has been updated by look pushed tag in the tagList. if
          // not found return
          if (opts.tagList) {
        ...
        alreadyInList = false;
        // use jQuery.map to make this work in IE8 (pure JS map is JS 1.6 but
        // IE8 only supports JS 1.5)
        ...
        if (alreadyInList) {
          ...
          if (!ignoreEvents)
            if (opts.AjaxPush !== null && opts.AjaxPushAllTags == null) {
              if ($.inArray(tag, opts.prefilled) === -1) {
                $.post(
                    opts.AjaxPush,
                    $.extend({tag: tag}, opts.AjaxPushParameters));
              }
            }
          ...
          html = '<span class="' + privateMethods.tagClasses.call($self) +
              '" id="' + newTagId + '">';
          ...
          if (opts.tagsContainer !== null) {
            ...
            if (tlid.length > 1) {
              lastTagObj =
                  $('#' + $self.data('tm_rndid') + '_' + tlid[tlid.length - 2]);
              lastTagObj.after($el);
            } else {
              $self.before($el);
            }
          ...
          // if (tagManagerOptions.maxTags > 0 && tlis.length >=
          // tagManagerOptions.maxTags) {
          //  obj.hide();
          //}
        }
        $self.val('');
      ...
      popTag: function() {
        var $self = $(this), tagId, tagBeingRemoved, tlis = $self.data('tlis'),
            tlid = $self.data('tlid');
    ...
    privateMethods = {
      ...
      refreshHiddenTagList: function() {
        ...
        $self.trigger(
            'tm:refresh', tlis.join($self.data('opts').baseDelimiter));
      },
        ...
        $.each(pta, function(key, val) {
          if (opts.externalTagId === true) {
            publicMethods.pushTag.call(
                $self, val[opts.prefillValueFieldName], true,
      ...
      spliceTag: function(tagId) {
        var $self = this, tlis = $self.data('tlis'), tlid = $self.data('tlid'),
            idx = $.inArray(tagId, tlid), tagBeingRemoved;
      ...
      init: function(options) {
        ...
        delimiters =
            opts.delimeters || opts.delimiters;  // 'delimeter' is deprecated
        keyNums = [
          9, 13, 17, 18, 19, 37, 38, 39, 40
        ...
        this.each(function() {
          ...
          if (opts.output === null) {
            hiddenObj =
                $('<input/>', {type: 'hidden', name: opts.hiddenTagListName});
          ...
          if (opts.prefilled !== null) {
            if (typeof (opts.prefilled) === 'object') {
              ...
              privateMethods.prefill.call(
                  $self, opts.prefilled.split(opts.baseDelimiter));
            ...
            privateMethods.prefill.call(
                $self, $(opts.output).val().split(opts.baseDelimiter));
          }
        });
