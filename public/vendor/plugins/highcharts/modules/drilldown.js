...
function t(a, b, d) {
  return 'rgba(' +
      [
        ...
        Math.round(a[2] + (b[2] - a[2]) * d), a[3] + (b[3] - a[3]) * d
      ].join(',') +
      ')'
}
var u = function() {}, o = g.getOptions(), i = g.each, p = g.extend,
    z = g.format, A = g.pick, q = g.wrap, l = g.Chart, n = g.seriesTypes,
    v = n.pie, m = n.column, w = HighchartsAdapter.fireEvent,
    x = HighchartsAdapter.inArray, r = [];
...
g.SVGRenderer.prototype.Element.prototype.fadeIn = function(a) {
  this.attr({opacity: 0.1, visibility: 'inherit'})
      .animate({opacity: A(this.newOpacity, 1)}, a || {duration: 250})
};
...
l.prototype.addSingleSeriesAsDrilldown = function(a, b) {
  ...
  i(d.chart.series, function(a) {
    if (a.xAxis === c)
      y.push(a), g.push(a.userOptions), a.levelNumber = a.levelNumber || k
  });
  h = {
    levelNumber: k,
  ...
  h.levelNumber = k + 1;
  if (c)
    c.oldPos = c.pos, c.userMin = c.userMax = null,
    f.userMin = f.userMax = null;
  if (d.type === h.type)
    h.animate = h.animateDrilldown || u, h.options.animation = !0
};
l.prototype.applyDrilldown = function() {
  var a = this.drilldownLevels, b;
  if (a && a.length > 0)
    b = a[a.length - 1].levelNumber, i(this.drilldownLevels, function(a) {
...
l.prototype.getDrilldownBackText = function() {
  ...
  if (a && a.length > 0)
    return a = a[a.length - 1], a.series = a.seriesOptions,
           z(this.options.lang.drillUpText, a)
};
l.prototype.showDrillUpButton = function() {
  var a = this, b = this.getDrilldownBackText(),
      d = a.options.drilldown.drillUpButton, c, f;
  this.drillUpButton ?
       ...
       this.drillUpButton =
           this.renderer
               .button(
                   ...
                   c, f && f.hover, f && f.select)
               .attr({align: d.position.align, zIndex: 9})
               .add()
               .align(d.position, !1, d.relativeTo || 'plotBox'))
};
l.prototype.drillUp = function() {
  for (var a = this, b = a.drilldownLevels, d = b[b.length - 1].levelNumber,
           c = b.length, f = a.series, h = f.length, e, g, j, k,
           l =
               function(b) {
                 var c;
                 i(f, function(a) {
    ...
    if (e = b[c], e.levelNumber === d) {
      ...
      g.remove(!1);
      if (j.xAxis)
        k = e.oldExtremes, j.xAxis.setExtremes(k.xMin, k.xMax, !1),
...
m.prototype.animateDrilldown = function(a) {
  var b = this, d = this.chart.drilldownLevels,
...
m.prototype.animateDrillupFrom = function(a) {
  ...
  i(this.points, function(c) {
    var h = c.graphic, e = g.Color(c.color).rgba, i = g.Color(a.color).rgba,
        j = function() {
          h.destroy();
          d && (d = d.destroy())
        };
    h &&
        (delete c.graphic,
         b ? h.animate(a.shapeArgs, g.merge(b, {
           step: function(a, b) {
             b.prop === 'start' && e.length === 4 && i.length === 4 &&
                 this.attr({fill: t(e, i, b.pos)})
           },
           complete: j
         })) :
...
v && p(v.prototype, {
  ...
  animateDrilldown: function(a) {
    var b = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1],
        d = this.chart.options.drilldown.animation, c = b.shapeArgs,
        f = c.start, h = (c.end - f) / this.points.length,
        e = g.Color(b.color).rgba;
    if (!a)
      i(this.points, function(a, b) {
        var i = g.Color(a.color).rgba;
        a.graphic.attr(g.merge(
            c,
            {start: f + b * h, end: f + (b + 1) * h}))[d ? 'animate' : 'attr'](
            a.shapeArgs, g.merge(d, {
              step: function(a, b) {
                b.prop === 'start' && e.length === 4 && i.length === 4 &&
                    this.attr({fill: t(e, i, b.pos)})
              }
...
g.Point.prototype.doDrilldown = function(a) {
  for (var b = this.series.chart, d = b.options.drilldown,
           c = (d.series || []).length, f;
       c-- && !f;)
    d.series[c].id === this.drilldown && x(this.drilldown, r) === -1 &&
        (f = d.series[c], r.push(this.drilldown));
  w(b, 'drilldown', {point: this, seriesOptions: f});
  f &&
      (a ? b.addSingleSeriesAsDrilldown(this, f) :
           b.addSeriesAsDrilldown(this, f))
};
q(g.Point.prototype, 'init', function(a, b, d, c) {
  var f = a.call(this, b, d, c), h = b.chart,
...
q(g.Series.prototype, 'drawDataLabels', function(a) {
  ...
  i(this.points, function(a) {
    if (a.drilldown && a.dataLabel)
      a.dataLabel.attr({'class': 'highcharts-drilldown-data-label'})
          .css(b)
          .on('click', function() {
            a.doDrilldown()
          })
...
var s, o = function(a) {
  ...
  i(this.points, function(a) {
    a.drilldown && a.graphic &&
        a.graphic.attr({'class': 'highcharts-drilldown-point'}).css({
          cursor: 'pointer'
        })
  })
};
for (s in n)
  n[s].prototype.supportsDrilldown && q(n[s].prototype, 'drawTracker', o)
})(Highcharts);
