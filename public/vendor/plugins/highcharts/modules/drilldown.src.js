...
var noop = function() {}, defaultOptions = H.getOptions(), each = H.each,
    extend = H.extend, format = H.format, pick = H.pick, wrap = H.wrap,
    Chart = H.Chart, seriesTypes = H.seriesTypes, PieSeries = seriesTypes.pie,
    ColumnSeries = seriesTypes.column, fireEvent = HighchartsAdapter.fireEvent,
    inArray = HighchartsAdapter.inArray, dupes = [];
...
Chart.prototype.addSingleSeriesAsDrilldown = function(point, ddOptions) {
  var oldSeries = point.series, xAxis = oldSeries.xAxis,
      yAxis = oldSeries.yAxis, newSeries,
      color = point.color || oldSeries.color, pointIndex, levelSeries = [],
      levelSeriesOptions = [], level, levelNumber;
    ...
    each(this.drilldownLevels, function(level) {
      if (level.levelNumber === levelToRemove) {
        each(level.levelSeries, function(series) {
          if (series.levelNumber ===
              levelToRemove) {  // Not removed, not added as part of a
                                // multi-series drilldown
            series.remove(false);
          }
        });
      }
  ...
  if (!this.drillUpButton) {
    ...
    this.drillUpButton =
        this.renderer
            ...
            .attr({align: buttonOptions.position.align, zIndex: 9})
            .add()
            .align(
                buttonOptions.position, false,
...
Chart.prototype.drillUp = function() {
  var chart = this, drilldownLevels = chart.drilldownLevels,
      ...
      i = drilldownLevels.length, chartSeries = chart.series,
      seriesI = chartSeries.length, level, oldSeries, newSeries, oldExtremes,
...
ColumnSeries.prototype.animateDrilldown = function(init) {
  var series = this, drilldownLevels = this.chart.drilldownLevels,
      animateFrom =
  ...
  if (!init) {
    ...
    each(this.points, function(point) {
      if (point.graphic) {
        point.graphic.attr(animateFrom)
...
ColumnSeries.prototype.animateDrillupFrom = function(level) {
  ...
  each(this.points, function(point) {
    var graphic = point.graphic, startColor = H.Color(point.color).rgba,
        endColor = H.Color(level.color).rgba, complete = function() {
          graphic.destroy();
          if (group) {
            group = group.destroy();
          }
      ...
      if (animationOptions) {
        ...
        graphic.animate(level.shapeArgs, H.merge(animationOptions, {
          step: function(val, fx) {
            if (fx.prop === 'start' && startColor.length === 4 &&
                endColor.length === 4) {
  ...
  extend(PieSeries.prototype, {
    ...
    animateDrilldown: function(init) {
      var level =
              this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1],
          animationOptions = this.chart.options.drilldown.animation,
          animateFrom = level.shapeArgs, start = animateFrom.start,
        ...
        each(this.points, function(point, i) {
          ...
          point.graphic.attr(H.merge(animateFrom, {
            start: start + i * startAngle,
              ...
              point.shapeArgs, H.merge(animationOptions, {
                step: function(val, fx) {
                  if (fx.prop === 'start' && startColor.length === 4 &&
                      endColor.length === 4) {
                    this.attr(
...
H.Point.prototype.doDrilldown = function(_holdRedraw) {
  var series = this.series, chart = series.chart,
      drilldown = chart.options.drilldown, i = (drilldown.series || []).length,
      seriesOptions;

  while (i-- && !seriesOptions) {
    if (drilldown.series[i].id === this.drilldown &&
        inArray(this.drilldown, dupes) === -1) {
...
wrap(H.Point.prototype, 'init', function(proceed, series, options, x) {
  var point = proceed.call(this, series, options, x), chart = series.chart,
  ...
  if (point.drilldown) {
    ...
    /*wrap(point, 'importEvents', function (proceed) { // wrapping importEvents
    makes point.click event work if (!this.hasImportedEvents) {
                    proceed.call(this);
...
wrap(H.Series.prototype, 'drawDataLabels', function(proceed) {
  ...
  each(this.points, function(point) {
    if (point.drilldown && point.dataLabel) {
      point.dataLabel.attr({'class': 'highcharts-drilldown-data-label'})
          .css(css)
          .on('click', function() {
            point.doDrilldown();
          });
...
var type, drawTrackerWrapper = function(proceed) {
  ...
  each(this.points, function(point) {
    if (point.drilldown && point.graphic) {
      point.graphic.attr({'class': 'highcharts-drilldown-point'}).css({
        cursor: 'pointer'
      });
    }
  });
