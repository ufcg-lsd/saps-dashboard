...
function RGBColor(color_string) {
  ...
  var color_defs = [
    ...
    },
    {
      ...
      process: function(bits) {
        return [
          parseInt(bits[1] + bits[1], 16), parseInt(bits[2] + bits[2], 16),
...
//		 renderCallback: function => will call the function after the
//first render is completed 		 forceRedraw: function => will call the function on
//every frame, if it returns true, will redraw
this.canvg =
...
build() {
  ...
  svg.ImagesLoaded =
    ...
    this.Color = {
      ...
      EM: function(viewPort) {
        ...
        var fontSize =
            new svg.Property('fontSize', svg.Font.Parse(svg.ctx.font).fontSize);
    ...
    this.CreateFont =
        function(
            fontStyle, fontVariant, fontWeight, fontSize, fontFamily, inherit) {
      var f = inherit != null ?
          this.Parse(inherit) :
      ...
      return {
        fontFamily: fontFamily || f.fontFamily,
            ...
            fontVariant: fontVariant || f.fontVariant, toString: function() {
              return [
  ...
  svg.ToNumberArray =
      function(s) {
    var a =
        svg.trim(svg.compressSpaces((s || '').replace(/,/g, ' '))).split(' ');
    ...
    return path;
                  ...
                  svg.BoundingBox =
                      function(
                          x1, y1, x2,
                          y2) {  // pass in initial points if you want
    this.x1 = Number.NaN;
    this.y1 = Number.NaN;
    this.x2 = Number.NaN;
    this.y2 = Number.NaN;
    ...
    this.x =
      ...
      this.addPoint(bb.x2, bb.y2);
                                    ...
                                    this.addQuadraticCurve =
                                        function(p0x, p0y, p1x, p1y, p2x, p2y) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      this.transforms.push(transform);
                          ...
                          svg.AspectRatio =
                              function(
                                  ctx, aspectRatio, width, desiredWidth, height,
    ...
    this.attribute =
        function(name, createIfNotExists) {
    ...
    if (node != null && node.nodeType == 1) {  // ELEMENT_NODE
      ...
      if (this.attribute('class').hasValue()) {
        var classes =
            svg.compressSpaces(this.attribute('class').value).split(' ');
      ...
      if (this.attribute('id').hasValue()) {
        if (svg.Definitions[this.attribute('id').value] == null) {
          svg.Definitions[this.attribute('id').value] = this;
                                  ...
                                  svg.Element.RenderedElementBase = function(
                                                                        node) {
    ...
    this.path =
      ...
      if (markers != null) {
        ...
        if (this.style('marker-end').Definition.isUrl()) {
          ...
          marker.render(
              ctx, markers[markers.length - 1][0],
  ...
  svg.Element.svg = function(node) {
    ...
    this.setContext = function(ctx) {
      ...
      if (this.attribute('viewBox').hasValue()) {
        ...
        svg.AspectRatio(
            ctx, this.attribute('preserveAspectRatio').value,
            svg.ViewPort.width(), width, svg.ViewPort.height(), height, minX,
            minY, this.attribute('refX').value, this.attribute('refY').value);
  ...
  svg.Element.ellipse = function(node) {
    ...
    this.path = function(ctx) {
      ...
      if (ctx != null) {
        ...
        ctx.moveTo(cx, cy - ry);
        ctx.bezierCurveTo(
            cx + (KAPPA * rx), cy - ry, cx + rx, cy - (KAPPA * ry), cx + rx,
            cy);
        ctx.bezierCurveTo(
            cx + rx, cy + (KAPPA * ry), cx + (KAPPA * rx), cy + ry, cx,
            cy + ry);
        ctx.bezierCurveTo(
            cx - (KAPPA * rx), cy + ry, cx - rx, cy + (KAPPA * ry), cx - rx,
            cy);
  ...
  svg.Element.polyline = function(node) {
    ...
    this.path =
      ...
      for (var i = 0; i < this.points.length - 1; i++) {
        markers.push(
            [this.points[i], this.points[i].angleTo(this.points[i + 1])]);
      }
      markers.push([
  ...
  svg.Element.path = function(node) {
    ...
    d = d.replace(
        ...
        '$1 $2');  // separate commands from points
    d = d.replace(
        /([^\s])([MmZzLlHhVvCcSsQqTtAa])/gm,
        '$1 $2');  // separate commands from points
    d = d.replace(
        /([0-9])([+\-])/gm, '$1 $2');  // separate digits when no comma
    d = d.replace(
        /(\.[0-9]*)(\.)/gm, '$1 $2');  // separate digits when no comma
    d = d.replace(
        ...
        return p;
                                                      ...
                                                      this.addMarker =
                                                          function(
                                                              p, from,
                                                              priorTo) {
        // if the last angle isn't filled in because we didn't have this point
        // yet ...
        ...
        switch (pp.command.toUpperCase()) {
          ...
          case 'C':
            while (!pp.isCommandOrEnd()) {
              ...
              bb.addBezierCurve(
                  curr.x, curr.y, p1.x, p1.y, cntrl.x, cntrl.y, cp.x, cp.y);
              if (ctx != null)
          ...
          case 'Q':
            while (!pp.isCommandOrEnd()) {
              ...
              bb.addQuadraticCurve(
                  curr.x, curr.y, cntrl.x, cntrl.y, cp.x, cp.y);
              if (ctx != null)
          ...
          case 'A':
            while (!pp.isCommandOrEnd()) {
              ...
              if (isNaN(s)) s = 0;
              var cpp =
                  new svg.Point(s * rx * currp.y / ry, s * -ry * currp.x / rx);
              // cx, cy
              ...
              // initial angle
              var a1 =
                  a([1, 0], [(currp.x - cpp.x) / rx, (currp.y - cpp.y) / ry]);
              // angle delta
              ...
              pp.addMarkerAngle(
                  cp, ad + (sweepFlag == 0 ? 1 : -1) * Math.PI / 2);

              bb.addPoint(
                  cp.x, cp.y);  // TODO: this is too naive, make it better
              if (ctx != null) {
                var r = rx > ry ? rx : ry;
  ...
  svg.Element.pattern = function(node) {
    ...
    this.createPattern = function(ctx, element) {
      ...
      tempSvg.attributes['x'] =
          new svg.Property('x', this.attribute('x').value);
      tempSvg.attributes['y'] =
          new svg.Property('y', this.attribute('y').value);
  ...
  svg.Element.marker = function(node) {
    ...
    this.render = function(ctx, point, angle) {
      ...
      tempSvg.attributes['viewBox'] =
          new svg.Property('viewBox', this.attribute('viewBox').value);
      tempSvg.attributes['refX'] =
          new svg.Property('refX', this.attribute('refX').value);
      tempSvg.attributes['refY'] =
          new svg.Property('refY', this.attribute('refY').value);
      tempSvg.attributes['width'] =
  ...
  svg.Element.GradientBase = function(node) {
    ...
    this.getGradient =
      ...
      if (this.attribute('xlink:href').hasValue()) {
        stopsContainer =
            this.attribute('xlink:href').Definition.getDefinition();
      }

      var g = this.getGradient(ctx, element);
      for (var i = 0; i < stopsContainer.stops.length; i++) {
        g.addColorStop(
            stopsContainer.stops[i].offset, stopsContainer.stops[i].color);
      }
      ...
      if (this.attribute('gradientTransform').hasValue()) {
        ...
        rect.attributes['x'] =
            new svg.Property('x', -svg.MAX_VIRTUAL_PIXELS / 3.0);
        rect.attributes['y'] =
            new svg.Property('y', -svg.MAX_VIRTUAL_PIXELS / 3.0);
        rect.attributes['width'] =
            new svg.Property('width', svg.MAX_VIRTUAL_PIXELS);
        rect.attributes['height'] =
            new svg.Property('height', svg.MAX_VIRTUAL_PIXELS);
        ...
        tempSvg.attributes['height'] =
            new svg.Property('height', rootView.height);
  ...
  svg.Element.text = function(node) {
    ...
    this.setContext =
      ...
      for (var i = 0; i < this.children.length; i++) {
        ...
        if (textAnchor != 'start' &&
            (i == 0 || child.attribute('x').hasValue())) {  // new group?
    ...
    this.getGlyph =
      ...
      return glyph;
        ...
        this.renderChildren =
            function(ctx) {
      var customFont =
          this.parent.style('font-family').Definition.getDefinition();
      if (customFont != null) {
        var fontSize =
            this.parent.style('font-size')
                .numValueOrDefault(svg.Font.Parse(svg.ctx.font).fontSize);
        var fontStyle =
            this.parent.style('font-style')
    ...
    this.renderChildren =
      ...
      if (this.hasText) {
        ...
        svg.Mouse.checkBoundingBox(
            this,
            new svg.BoundingBox(
                this.x, this.y - fontSize.Length.toPixels('y'),
                this.x + this.measureText(ctx), this.y));
      } else {
  ...
  svg.Element.image = function(node) {
    ...
    this.renderChildren = function(ctx) {
      ...
      ctx.translate(x, y);
      svg.AspectRatio(
          ctx, this.attribute('preserveAspectRatio').value, width,
    ...
    this.setContext = function(ctx) {
      ...
      if (this.attribute('viewBox').hasValue()) {
        ...
        svg.AspectRatio(
            ...
            this.attribute('height').Length.toPixels('y'), height, minX, minY);
        ...
        for (var j = 0; j < cssClasses.length; j++) {
          ...
          if (cssClass != '') {
            ...
            for (var k = 0; k < cssProps.length; k++) {
              ...
              var value =
                  cssProps[k].substr(prop + 1, cssProps[k].length - prop);
  ...
  svg.Element.filter = function(node) {
    ...
    this.apply =
      ...
      // render on me
      ctx.drawImage(
          c, 0, 0, width + 2 * px, height + 2 * py, x - px, y - py,
            ...
            for (var i = 1; i < mask.length; i++) {
              ...
              sum += mask[i] *
                  ((a1 == 0 ?
                        255 :
                        imGet(
                            src, Math.max(x - i, 0), y, width, height, rgba)) *
                       (a1 == 0 || rgba == 3 ? 1 : a1) +
                   (a2 == 0 ? 255 :
                              imGet(
                                  src, Math.min(x + i, width - 1), y, width,
                                  height, rgba)) *
                       (a2 == 0 || rgba == 3 ? 1 : a2));
            }
  ...
  svg.Element.title = function(node) {} svg.Element.title.prototype =
      new svg.Element.ElementBase;

  // desc element, do nothing
  svg.Element.desc = function(node) {} svg.Element.desc.prototype =
      new svg.Element.ElementBase;
    ...
    var draw =
      ...
      if (svg.opts != null && svg.opts['scaleWidth'] != null &&
        ...
        if (e.attribute('width').hasValue())
          xRatio = e.attribute('width').Length.toPixels('x') /
              svg.opts['scaleWidth'];
        if (e.attribute('height').hasValue())
          yRatio = e.attribute('height').Length.toPixels('y') /
...
var UNDEFINED, DIV = 'div', ABSOLUTE = 'absolute', RELATIVE = 'relative',
               HIDDEN = 'hidden', VISIBLE = 'visible', PX = 'px',
               css = Highcharts.css, CanVGRenderer = Highcharts.CanVGRenderer,
               SVGRenderer = Highcharts.SVGRenderer, extend = Highcharts.extend,
               merge = Highcharts.merge, addEvent = Highcharts.addEvent,
...
extend(CanVGRenderer.prototype, {
  ...
  setContainer: function(container, chartWidth, chartHeight) {
    var containerStyle = container.style,
        ...
        containerLeft = containerStyle.left, containerTop = containerStyle.top,
        containerOffsetWidth = container.offsetWidth,
        containerOffsetHeight = container.offsetHeight, canvas,
    ...
    canvas = createElement(
        'canvas', {width: containerOffsetWidth, height: containerOffsetHeight},
        {position: RELATIVE, left: containerLeft, top: containerTop},
        container);
    this.canvas = canvas;

    // Create the tooltip line and div, they are placed as siblings to
    // the container (and as direct childs to the div specified in the html
    // page)
    ...
    // Move away the svg node to a new div inside the container's parent so we
    // can hide it.
    var hiddenSvg = createElement(
        DIV, {width: containerOffsetWidth, height: containerOffsetHeight},
        {visibility: HIDDEN, left: containerLeft, top: containerTop},
        containerParent);
  ...
  /**
   * Configures the renderer with the chart. Attach a listener to the event
   *tooltipRefresh.
   **/
  configure: function(chart) {
    var renderer = this, options = chart.options.tooltip,
        borderWidth = options.borderWidth, tooltipDiv = renderer.ttDiv,
        tooltipDivStyle = options.style, tooltipLine = renderer.ttLine,
    ...
    addEvent(chart, 'tooltipRefresh', function(args) {
      var chartContainer = chart.container,
          ...
          offsetTop = chartContainer.offsetTop, position;

      // Set the content of the tooltip
      tooltipDiv.innerHTML = args.text;

      // Compute the best position for the tooltip based on the divs size and
      // container size.
      position = chart.tooltip.getPosition(
          tooltipDiv.offsetWidth, tooltipDiv.offsetHeight,
  ...
  /**
   * Extend SVGRenderer.destroy to also destroy the elements added by
   * CanVGRenderer.
   */
  destroy: function() {
    var renderer = this;
  ...
  /**
   * Take a color and return it if it's a string, do not make it a gradient even
   * if it is a gradient. Currently canvg cannot render gradients (turns out
   * black), see: http://code.google.com/p/canvg/issues/detail?id=104
   *
