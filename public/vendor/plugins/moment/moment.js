...
var moment,
    ...
    hasOwnProperty = Object.prototype.hasOwnProperty, i,

    YEAR = 0, MONTH = 1, DATE = 2, HOUR = 3, MINUTE = 4, SECOND = 5,
    MILLISECOND = 6,
    ...
    isoDates =
        [
          ...
          ['GGGG-[W]WW-E', /\d{4}-W\d{2}-\d/], ['GGGG-[W]WW', /\d{4}-W\d{2}/],
          ['YYYY-DDD', /\d{4}-\d{3}/]
        ],
...
function Duration(duration) {
  var normalizedInput = normalizeObjectUnits(duration),
      ...
      months = normalizedInput.month || 0, weeks = normalizedInput.week || 0,
      days = normalizedInput.day || 0, hours = normalizedInput.hour || 0,
...
function addOrSubtractDurationFromMoment(
  ...
  var milliseconds = duration._milliseconds, days = duration._days,
...
function compareArrays(array1, array2, dontConvert) {
  var len = Math.min(array1.length, array2.length),
      lengthDiff = Math.abs(array1.length - array2.length), diffs = 0, i;
  for (i = 0; i < len; i++) {
...
function isValid(m) {
  if (m._isValid == null) {
    m._isValid = !isNaN(m._d.getTime()) && m._pf.overflow < 0 && !m._pf.empty &&
        !m._pf.invalidMonth && !m._pf.nullInput && !m._pf.invalidFormat &&
...
function loadLocale(name) {
  ...
  if (!locales[name] && hasModule) {
    try {
      ...
      require('./locale/' + name);
      // because defineLocale currently also sets the global locale, we want to
      // undo that for lazy loaded locales
...
function dateFromObject(config) {
  ...
  config._a = [
    ...
    normalizedInput.hour, normalizedInput.minute, normalizedInput.second,
...
function makeDateFromStringAndFormat(config) {
  ...
  var string = '' + config._i, i, parsedInput, tokens, token, skipped,
...
function relativeTime(posNegDuration, withoutSuffix, locale) {
  var duration = moment.duration(posNegDuration).abs(),
      seconds = round(duration.as('s')), minutes = round(duration.as('m')),
      hours = round(duration.as('h')), days = round(duration.as('d')),
      ...
      days === 1 && ['d'] || days < relativeTimeThresholds.d && ['dd', days] ||
      months === 1 && ['M'] ||
...
function weekOfYear(mom, firstDayOfWeek, firstDayOfWeekOfYear) {
  var end = firstDayOfWeekOfYear - firstDayOfWeek,
      daysToDayOfWeek = firstDayOfWeekOfYear - mom.day(), adjustedMoment;
...
extend(moment.fn = Moment.prototype, {
  ...
  toArray: function() {
    var m = this;
    return [
      m.year(), m.month(), m.date(), m.hours(), m.minutes(), m.seconds(),
      m.milliseconds()
    ];
  },
  ...
  diff: function(input, units, asFloat) {
    var that = makeAs(input, this),
        zoneDiff = (this.zone() - that.zone()) * 6e4, diff, output, daysAdjust;
  ...
  calendar: function(time) {
    ...
    var now = time || moment(), sod = makeAs(now, this).startOf('day'),
  ...
  isAfter: function(input, units) {
    units =
        normalizeUnits(typeof units !== 'undefined' ? units : 'millisecond');
  ...
  isSame: function(input, units) {
    ...
    if (units === 'millisecond') {
      ...
      return +this.clone().startOf(units) ===
          +makeAs(input, this).startOf(units);
    }
  },
      ...
      if (offset !== input) {
        if (!keepLocalTime || this._changeInProgress) {
          addOrSubtractDurationFromMoment(
              this, moment.duration(offset - input, 'm'), 1, false);
  ...
  weekYear: function(input) {
    var year =
        weekOfYear(
            this, this.localeData()._week.dow, this.localeData()._week.doy)
            .year;
...
extend(moment.duration.fn = Duration.prototype, {
  ...
  _bubble: function() {
    var milliseconds = this._milliseconds, days = this._days,
        months = this._months, data = this._data, seconds, minutes, hours,
        years = 0;
  ...
  toISOString: function() {
    ...
    var years = Math.abs(this.years()), months = Math.abs(this.months()),
        days = Math.abs(this.days()), hours = Math.abs(this.hours()),
    ...
    return (this.asSeconds() < 0 ? '-' : '') + 'P' +
        ...
        (days ? days + 'D' : '') + ((hours || minutes || seconds) ? 'T' : '') +
