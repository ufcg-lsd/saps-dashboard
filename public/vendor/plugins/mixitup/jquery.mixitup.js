...
$.MixItUp.prototype = {
  ...
  _init: function(domNode, config) {
    ...
    self._$parent = self._$targets.parent().length ? self._$targets.parent() :
                                                     self._$container;
    ...
    self._activeFilter = self.load.filter === 'all' ?
        ...
        self.load.filter === 'none' ? '' : self.load.filter;
    ...
    if (self.controls.toggleFilterButtons) {
      ...
      for (var i = 0; i < self._toggleArray.length; i++) {
        self._updateControls(
            {filter: self._toggleArray[i], sort: self._activeSort}, true);
      };
    } else if (self.controls.enable) {
      self._updateControls(
          {filter: self._activeFilter, sort: self._activeSort});
    }
  ...
  _platformDetect: function() {
    var self = this, vendorsTrans = ['Webkit', 'Moz', 'O', 'ms'],
        vendorsRAF = ['webkit', 'moz'],
        chrome = window.navigator.appVersion.match(/Chrome\/(\d+)\./) || false,
        ff = typeof InstallTrigger !== 'undefined', prefix = function(el) {
      ...
      if (target.dataset === undf || force) {
        ...
        for (var j = 0; j < target.attributes.length; j++) {
          ...
          if (name.indexOf('data-') > -1) {
            var dataName =
                self._helpers._camelCase(name.substring(5, name.length));
  ...
  _bindHandlers: function() {
    var self = this, filters = $.MixItUp.prototype._bound._filter,
    ...
    if (self.controls.live) {
      self._$body
          .on('click.mixItUp.' + self._id, self.selectors.sort,
              function() {
  ...
  _processClick: function($button, type) {
    var self = this, trackClick = function($button, type, off) {
      ...
      if (proto._handled['_' + type][self.selectors[type]] ===
    ...
    if (!self._mixing ||
      ...
      if (type === 'filter') {
        var filter = $button.attr('data-filter'), ndx,
  ...
  _updateControls: function(command, multi) {
    var self = this, output = {filter: command.filter, sort: command.sort},
        update = function($el, filter) {
          (multi && type === 'filter' &&
           !(output.filter === 'none' || output.filter === '')) ?
              $el.filter(filter).addClass(self.controls.activeClass) :
              $el.removeClass(self.controls.activeClass)
                  .filter(filter)
    ...
    for (var i = 0; i < 2; i++) {
      $el = self.controls.live ? $(self.selectors[type]) :
                                 self['_$' + type + 'Buttons'];
  ...
  _compare: function(a, b, depth) {
    ...
    var self = this, order = self._newSort[depth].order,
        getData =
            function(el) {
          return el.dataset[self._newSort[depth].sortBy] || 0;
        },
        attrA =
            isNaN(getData(a) * 1) ? getData(a).toLowerCase() : getData(a) * 1,
        attrB =
            isNaN(getData(b) * 1) ? getData(b).toLowerCase() : getData(b) * 1;
  ...
  _printSort: function(reset) {
    var self = this, order = reset ? self._startOrder : self._newOrder,
    ...
    for (var i = 0; i < order.length; i++) {
      ...
      if (self._newSort[0].sortBy === 'default' &&
          self._newSort[0].order === 'desc' && !reset) {
    ...
    nextSibling ? self._$parent[0].insertBefore(frag, nextSibling) :
  ...
  _parseSort: function(sortString) {
    var self = this,
        rules = typeof sortString === 'string' ? sortString.split(' ') :
                                                 [sortString],
        newSort = [];

    for (var i = 0; i < rules.length; i++) {
      var rule = typeof sortString === 'string' ? rules[i].split(':') :
                                                  ['custom', rules[i]],
          ruleObj = {
  ...
  _parseEffects: function() {
    var self = this,
        ...
        parse = function(effect, extract, reverse) {
          ...
          if (invert) {
            return value.charAt(0) === '-' ? value.substr(1, value.length) :
                                             '-' + value;
          } else {
            return value;
          }
          ...
          for (var i = 0; i < transforms.length; i++) {
            var prop = transforms[i][0], def = transforms[i][1],
                inverted = invert && prop !== 'scale';

            effects[key] += parse(prop) ? prop + '(' +
    ...
    self.animation.reverseOut ? buildTransform('transformOut', true) :
    ...
    effects.transition = self._getPrefixedCSS(
        'transition',
        'all ' + self.animation.duration + 'ms ' + self.animation.easing +
            ', opacity ' + self.animation.duration + 'ms linear');

    self.animation.stagger = parse('stagger') ? true : false;
    self.animation.staggerDuration = parseInt(
        parse('stagger') ?
            (parse('stagger', true).val ? parse('stagger', true).val : 100) :
            100);
  ...
  _buildState: function(future) {
    ...
    state = {
      ...
      activeSort: future && self._newSortString ? self._newSortString :
                                                  self._activeSort,
      ...
      display: future && self._newDisplay ? self._newDisplay :
                                            self.layout.display
    };
  ...
  _goMix: function(animate) {
    ...
    if (typeof self.callbacks.onMixStart === 'function') {
      self.callbacks.onMixStart.call(
          self._domNode, self._state, futureState, self);
    }
  ...
  _getOrigMixData: function() {
    ...
    self._startHeight =
        self._incPadding ? self._$parent.outerHeight() : self._$parent.height();
  ...
  _setInter: function() {
    ...
    if (self._changingLayout && self.animation.animateChangeLayout) {
      ...
      if (self._changingClass) {
        self._$container.removeClass(self.layout.containerClass)
  ...
  _getFinalMixData: function() {
    ...
    if (self._changingClass && self.animation.animateChangeLayout) {
      self._$container.removeClass(self._newClass)
    ...
    for (var i = 0; i < self._$toShow.length; i++) {
      ...
      if (self.animation.animateResizeTargets) {
        ...
        el.style.marginRight =
            ...
            (el.dataset.finalMarginRight * 1) + 'px';
      ...
      if (self.animation.animateResizeTargets) {
        ...
        if (el.dataset.origHeight - el.dataset.finalHeight) {
          el.style.marginBottom =
              ...
              (el.dataset.origMarginBottom * 1) + 'px';
        }
      }
    }
  ...
  _animateTargets: function() {
    ...
    self._$parent
        .css(self._getPrefixedCSS(
            'perspective', self.animation.perspectiveDistance + 'px'))
        .css(self._getPrefixedCSS(
            'perspective-origin', self.animation.perspectiveOrigin));

    if (self.animation.animateResizeContainer) {
      self._$parent
          .css(self._getPrefixedCSS(
              'transition', 'height ' + self.animation.duration + 'ms ease'))
    ...
    for (var i = 0; i < self._$toHide.length; i++) {
      var el = self._$toHide[i], $el = $(el), delay = self._getDelay(i),
          toHideCSS = {};
  ...
  _cleanUp: function() {
    ...
    !self._changingLayout ? self._$show.css('display', self.layout.display) :
                            self._$show.css('display', self._newDisplay);

    self._$targets.css(self._brake);

    self._$targets.removeStyle(targetStyles, self._prefix)
        .removeAttr(
    ...
    if (self._changingLayout) {
      ...
      if (self._changingClass) {
        self._$parent.removeClass(self.layout.containerClass)
            .addClass(self._newClass);
  ...
  _getPrefixedCSS: function(property, value, prefixValue) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      prefixValue ? styles[prefix + property] = prefix + value :
  ...
  _getDelay: function(i) {
    var self = this,
        n = typeof self.animation.staggerSequence === 'function' ?
  ...
  _execAction: function(methodName, isPost, args) {
    ...
    if (!self._actions.isEmptyObject &&
  ...
  _execFilter: function(methodName, value, args) {
    ...
    if (!self._filters.isEmptyObject &&
  ...
  multiMix: function() {
    ...
    if (!self._mixing) {
      ...
      var sort = args.command.sort, filter = args.command.filter,
      ...
      if (changeLayout) {
        self._newDisplay = (typeof changeLayout === 'string') ?
            changeLayout :
      ...
      self._goMix(
          args.animate ^ self.animation.enable ? args.animate :
                                                 self.animation.enable);

      self._execAction('multiMix', 1, arguments);

    } else {
      if (self.animation.queue &&
          self._queue.length < self.animation.queueLimit) {
    ...
    var self = this, args = self._parseInsertArgs(arguments),
        ...
        target = (function() {
          ...
          if (self._$targets.length) {
            return (args.index < self._$targets.length ||
                    !self._$targets.length) ?
  ...
  append: function() {
    ...
    self.insert(
        self._state.totalTargets, args.$object, args.multiMix, args.callback);
  },
  ...
  getOption: function(string) {
    var self = this, getProperty = function(obj, prop) {
      var parts = prop.split('.'), last = parts.pop(), l = parts.length, i = 1,
  ...
  destroy: function(hideAll) {
    ...
    self._$body.add($(self.selectors.sort))
...
$.fn.removeStyle = function(style, prefix) {
  ...
  return this.each(function() {
    ...
    for (var i = 0; i < styles.length; i++) {
      for (var j = 0; j < 4; j++) {
        switch (j) {
          ...
          case 3:
            var prop =
                $.MixItUp.prototype._helpers._camelCase(prefix + styles[i]);
        }

        if (el.style[prop] !== undf && typeof el.style[prop] !== 'unknown' &&
    ...
    if (el.attributes && el.attributes.style && el.attributes.style !== undf &&
