...
var Mapplic = function() {
  ...
  self.init =
    ...
    self.el =
        el.addClass('mapplic-element mapplic-loading').height(self.o.height);
    ...
    $.getJSON(self.o.source, function(data) {  // Success
     ...
     }).fail(function() {  // Failure: couldn't load JSON file, or it is
                           // invalid.
      console.error(
  ...
  Tooltip() {
    ...
    this.init =
      ...
      $('<a></a>')
          ...
          .appendTo(this.el);
      this.image =
          $('<img>').addClass('mapplic-tooltip-image').hide().appendTo(this.el);
      this.title =
          $('<h4></h4>').addClass('mapplic-tooltip-title').appendTo(this.el);
      ...
      this.el.css({left: x + '%', top: y + '%', marginTop: mt, marginLeft: ml});
  ...
  HoverTooltip() {
    ...
    this.init =
      ...
      var x = location.x * 100, y = location.y * 100,
  ...
  Minimap() {
    ...
    this.init =
        function() {
      this.el =
          $('<div></div>').addClass('mapplic-minimap').appendTo(self.container);
      ...
      var width = Math.round(
          ...
          left =
              Math.round(-x / self.contentWidth / self.scale * this.el.width()),
  ...
  Sidebar() {
    ...
    this.init =
      ...
      var listContainer =
          $('<div></div>').addClass('mapplic-list-container').appendTo(this.el);
      this.list =
          $('<ol></ol>').addClass('mapplic-list').appendTo(listContainer);
      this.notfound = $('<p></p>')
      ...
      $.each(categories, function(index, category) {
        ...
        var ol =
            $('<ol></ol>').css('border-color', category.color).appendTo(item);
      ...
      // Count
      $('.mapplic-list-count', category)
          .text($('.mapplic-list-shown', category).length);
    }
  ...
  var processData =
    ...
    self.container =
        $('<div></div>').addClass('mapplic-container').appendTo(self.el);
    self.map =
        $('<div></div>').addClass('mapplic-map').appendTo(self.container);
    ...
    if (data.mapheight / self.container.height() >
        data.mapwidth / self.container.width()) {
    ...
    self.map.css({'width': data.mapwidth, 'height': data.mapheight});
    ...
    if (data.levels) {
      $.each(data.levels, function(index, value) {
        ...
        var extension =
            source.substr((source.lastIndexOf('.') + 1)).toLowerCase();
  ...
  var addControls =
      function() {
    var map = self.map, mapbody = $('.mapplic-map-image', self.map);
    ...
    mapbody.on('touchstart', function(e) {
      var orig = e.originalEvent, pos = map.position();
      ...
      mapbody.on('touchmove', function(e) {
        ...
        if (touches == 1) {
          self.x =
              normalizeX(orig.changedTouches[0].pageX - map.data('touchX'));
          self.y =
              normalizeY(orig.changedTouches[0].pageY - map.data('touchY'));
    ...
    mapPinch.on('touch transform', function(ev) {
      switch (ev.type) {
        ...
        case 'transform':
          ...
          self.x = normalizeX(
              self.x -
              (center.pageX - self.container.offset().left - self.x) *
                  (self.scale / oldscale - 1));
          self.y = normalizeY(
              self.y -
              (center.pageY - self.container.offset().top - self.y) *
                  (self.scale / oldscale - 1));
  ...
  var level =
    ...
    switch (target) {
      ...
      case '-':
        ...
        break;
      default:
        $('option[value="' + target + '"]', self.levelselect)
            .prop('selected', 'selected');
    }
    ...
    $.each(self.data.levels, function(index, layer) {
      $.each(layer.locations, function(index, value) {
        if (value.id == id) {
          ...
          zoomTo(
              value.x, parseFloat(value.y) - drop, zoom, duration,
              'easeInOutCubic');
        }
      });
    });
  ...
  var zoomTo =
      function(x, y, s, duration, easing) {
    ...
    self.y = normalizeY(
        self.container.height() * 0.5 - self.scale * self.contentHeight * y);
...
$.fn.mapplic = function(params) {
  ...
  return this.each(function(index) {
    var me = $(this), key = 'mapplic' + (len > 1 ? '-' + ++index : ''),
