...
!function($) {
  ...
  var Markdown = function(element, options) {
    ...
    var opts = [
      ...
      'iconlibrary', 'language', 'footer', 'fullscreen', 'hiddenButtons',
      'disabledButtons'
    ];
    ...
    this.$editable =
        {el: null, type: null, attrKeys: [], attrValues: [], content: null};
    this.$options = $.extend(
        true, {}, $.fn.markdown.defaults, options,
  ...
  Markdown.prototype = {
      ...
      for (i = 0; i < buttonsArray.length; i++) {
        ...
        for (y = 0; y < btnGroups.length; y++) {
          ...
          for (z = 0; z < buttons.length; z++) {
            ...
            buttonContainer.text(' ' + this.__localize(btnText))
    ...
    ,
    __handle: function(e) {
      var target = $(e.currentTarget), handler = this.$handler,
          callback = this.$callback, handlerName = target.attr('data-handler'),
    ...
    showEditor: function() {
      var instance = this, textarea, ns = this.$ns, container = this.$element,
          ...
          originalWidth = container.css('width'), editable = this.$editable,
          handler = this.$handler, callback = this.$callback,
      ...
      if (this.$editor === null) {
        ...
        if (options.reorderButtonGroups.length > 0) {
          allBtnGroups = allBtnGroups
                             .filter(function(btnGroup) {
                               return options.reorderButtonGroups.indexOf(
                                          btnGroup.name) > -1;
                             })
                             .sort(function(a, b) {
        ...
        if (container.is('textarea')) {
          ...
          textarea =
              $('<textarea/>', {'class': 'md-input', 'val': currentContent});
        ...
        if (options.savable) {
          ...
          editorFooter.append(
              '<button class="btn btn-success" data-provider="' + ns +
              '" data-handler="' + saveHandler +
              '"><i class="icon icon-white icon-ok"></i> ' +
              this.__localize('Save') + '</button>');
        }

        footer = typeof options.footer === 'function' ? options.footer(this) :
                                                        options.footer;
        ...
        this.$editor.on(
            'click', '[data-provider="bootstrap-markdown"]',
        ...
        if (this.eventSupported('keydown') &&
          ...
          editorHeader.find('[data-provider="bootstrap-markdown"]')
              .each(function() {
    ...
    showPreview: function() {
      ...
      // Set the content based from the callback content if string otherwise
      // parse value from textarea
      content = typeof callbackContent == 'string' ? callbackContent :
                                                     this.parseContent();
    ...
    hidePreview: function() {
      ...
      var container =
          this.$editor.find('div[data-provider="markdown-preview"]');
    ...
    findSelection: function(chunk) {
      ...
      if (startChunkPosition = content.indexOf(chunk),
          startChunkPosition >= 0 && chunk.length > 0) {
        var oldSelection = this.getSelection(), selection;

        this.setSelection(
            startChunkPosition, startChunkPosition + chunk.length);
    ...
    focus: function(e) {
      var options = this.$options, isHideable = options.hideable,
          editor = this.$editor;
    ...
    blur: function(e) {
      var options = this.$options, isHideable = options.hideable,
          editor = this.$editor, editable = this.$editable;
        ...
        if (isHideable) {
          ...
          if (editable.el !== null) {
            ...
            var oldElement = $('<' + editable.type + '/>'),
                ...
                currentContent = (typeof markdown == 'object') ?
        ...
        data: [
          ...
          },
          {
            ...
            callback: function(e) {
              ...
              var chunk, cursor, selected = e.getSelection(),
                                 content = e.getContent(), pointer, prevChar;
  ...
  $(document)
      .on('click.markdown.data-api', '[data-provide="markdown-editable"]',
          function(e) {
            initMarkdown($(this));
            e.preventDefault();
          })
