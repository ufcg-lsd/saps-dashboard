...
var ElevateZoom = {
  init: function(options, elem) {
    ...
    $('#' + self.options.gallery + ' a').click(function(e) {
      ...
      if (self.options.galleryActiveClass) {
        $('#' + self.options.gallery + ' a')
            .removeClass(self.options.galleryActiveClass);
  ...
  startZoom: function() {
    ...
    if (self.options.imageCrossfade) {
      self.zoomWrap = self.$elem.wrap(
          '<div style="height:' + self.nzHeight + 'px;width:' + self.nzWidth +
    ...
    if (self.options.zoomType == 'window') {
      self.zoomWindowStyle = 'overflow: hidden;' +
          ...
          'px;float: left;' +
          'background-size: ' + self.largeWidth / self.currentZoomLevel +
          'px ' + self.largeHeight / self.currentZoomLevel + 'px;' +
          'display: none;z-index:100;' +
          'border: ' + String(self.options.borderSize) + 'px solid ' +
          self.options.borderColour + ';background-repeat: no-repeat;' +
          'position: absolute;';
    }
    ...
    self.zoomContainer = $(
        ...
        self.nzHeight + 'px;width:' + self.nzWidth + 'px;"></div>');
    ...
    if (self.options.zoomType != 'inner') {
      self.zoomLens = $('<div class=\'zoomLens\' style=\'' + self.lensStyle +
                        self.lensRound + '\'>&nbsp;</div>')
      ...
      if (self.options.tint) {
        ...
        self.zoomTintImage =
            $('<img style="position: absolute; left: 0px; top: 0px; max-width: none; width: ' +
              self.nzWidth + 'px; height: ' + self.nzHeight + 'px;" src="' +
              self.imageSrc + '">')
    ...
    if (isNaN(self.options.zoomWindowPosition)) {
      self.zoomWindow =
          $('<div style=\'z-index:999;left:' + (self.windowOffsetLeft) +
            'px;top:' + (self.windowOffsetTop) + 'px;' + self.zoomWindowStyle +
    ...
    //  self.captionStyle = "text-align: left;background-color: black;color:
    //  white;font-weight: bold;padding: 10px;font-family: sans-serif;font-size:
    //  11px";
    // self.zoomCaption = $('<div class="elevatezoom-caption"
    // style="'+self.captionStyle+'display: block; width: 280px;">INSERT ALT
    // TAG</div>').appendTo(self.zoomWindow.parent());
    ...
    if (self.options.scrollZoom) {
      self.zoomContainer.add(self.$elem)
          .bind('mousewheel DOMMouseScroll MozMousePixelScroll', function(e) {
            //						in IE there is issue with firing of mouseleave - So
            //check whether still scrolling 						and on mouseleave check if
            //scrolllock
  ...
  setPosition: function(e) {
    ...
    // set responsive
    // will checking if the image needs changing before running this code work
    // faster?
    if (self.options.responsive && !self.options.scrollZoom) {
      if (self.options.showLens) {
        ...
        if (self.options.zoomType != 'lens') {
          ...
          // if the lens is heigher than the image, then set lens size to image
          // size
          if (self.nzHeight < self.options.zoomWindowWidth / self.widthRatio) {
            lensHeight = self.nzHeight;
    ...
    if (self.options.zoomType == 'inner') {
      ...
      self.Eroppos =
          (self.mouseLeft >
           (self.nzWidth - (self.nzWidth / 2) / self.widthRatio -
            (self.options.lensBorderSize * 2)));
    }

    // if the mouse position of the slider is one of the outerbounds, then hide
    // window and lens
    if (self.mouseLeft <= 0 || self.mouseTop < 0 ||
        self.mouseLeft > self.nzWidth || self.mouseTop > self.nzHeight) {
      self.setElements('hide');
      return;
    }
    // else continue with operations
      ...
      // adjust the background position if the mouse is in one of the outer
      // regions
      ...
      if (self.options.zoomType == 'window') {
        if (self.Eboppos) {
          self.lensTopPos = Math.max(
              (self.nzHeight) - self.zoomLens.height() -
                  (self.options.lensBorderSize * 2),
              0);
        }
        if (self.Eroppos) {
          self.lensLeftPos =
              (self.nzWidth - (self.zoomLens.width()) -
               (self.options.lensBorderSize * 2));
        }
      }
      ...
      if (self.options.zoomType == 'inner') {
        if (self.Eboppos) {
          self.lensTopPos = Math.max(
              ((self.nzHeight) - (self.options.lensBorderSize * 2)), 0);
        }
        if (self.Eroppos) {
          self.lensLeftPos =
              (self.nzWidth - (self.nzWidth) -
               (self.options.lensBorderSize * 2));
        }
      }
      // if lens zoom
      ...
      if (self.options.showLens) {
        ...
        self.zoomLens.css(
            {left: self.lensLeftPos + 'px', top: self.lensTopPos + 'px'})
      }
    ...
    if (change == 'show') {
      if (!self.isWindowActive) {
        if (self.options.zoomWindowFadeIn) {
          self.zoomWindow.stop(true, true, false)
    ...
    if (change == 'hide') {
      if (self.isWindowActive) {
        if (self.options.zoomWindowFadeOut) {
          self.zoomWindow.stop(true, true)
    ...
    if (change == 'show') {
      if (!self.isTintActive) {
        if (self.options.zoomTintFadeIn) {
          self.zoomTint.css({opacity: self.options.tintOpacity})
              .animate()
              .stop(true, true)
              .fadeIn('slow');
        } else {
  ...
  setWindowPostition: function(e) {
    ...
    if (!isNaN(self.options.zoomWindowPosition)) {
      switch (self.options.zoomWindowPosition) {
        case 1:                                                    // done
          ...
          break;
        case 2:
          if (self.options.zoomWindowHeight >
              self.nzHeight) {  // positive margin

            self.windowOffsetTop =
                ((self.options.zoomWindowHeight / 2) - (self.nzHeight / 2)) *
                (-1);
            self.windowOffsetLeft = (self.nzWidth);  // DONE 1, 2, 3, 4, 16
        ...
        case 3:  // done
          self.windowOffsetTop =
              (self.nzHeight - self.zoomWindow.height() -
        ...
        case 5:                                    // done
          ...
          self.windowOffsetLeft =
              (self.nzWidth - self.zoomWindow.width() -
        ...
        case 6:
          if (self.options.zoomWindowHeight >
            ...
            self.windowOffsetLeft =
                ((self.options.zoomWindowWidth / 2) - (self.nzWidth / 2) +
        ...
        case 8:                                    // done
          self.windowOffsetTop = (self.nzHeight);  // DONE - 4,5,6,7,8
          self.windowOffsetLeft =
              (self.zoomWindow.width() + (self.options.borderSize * 2)) *
              (-1);  // DONE 8,9,10,11,12
          break;
        case 9:  // done
          self.windowOffsetTop =
        ...
        case 12:  // done
          self.windowOffsetTop =
              (self.zoomWindow.height() + (self.options.borderSize * 2)) *
    ...
    if (self.Eboppos) {
      self.windowTopPos = (self.largeHeight / self.currentZoomLevel -
...
self.bgxp += (self.largeWidth/self.newvalue  - self.bgxp) /
self.options.easingAmount; self.bgyp += (self.largeHeight/self.newvalue  -
...
self.zoomWindow.css({ "background-size": self.bgxp + 'px ' + self.bgyp + 'px'
        ...
        if (self.changeBgSize) {
          if (self.nzHeight > self.nzWidth) {
            ...
            if ((self.largeHeight / self.newvaluewidth) <
  ...
  setTintPosition: function(e) {
    ...
    if (self.Eboppos) {
      self.tintposy = (self.nzHeight - self.zoomLens.height() -
                       (self.options.lensBorderSize * 2)) *
          (-1);
    }
    if (self.Eroppos) {
  ...
  swaptheimage: function(smallimage, largeimage) {
    ...
    if (self.options.loadingIcon) {
      self.spinner = $(
          '<div style="background: url(\'' + self.options.loadingIcon +
          '\') no-repeat center;height:' + self.nzHeight +
          'px;width:' + self.nzWidth +
  ...
  changeZoomLevel: function(value) {
    ...
    if (self.options.zoomType == 'inner') {
      maxheightnewvalue =
          parseFloat(self.largeHeight / self.nzHeight).toFixed(2);
    ...
    if (scrcontinue) {
      ...
      if (((self.options.zoomWindowHeight) / self.heightRatio) <=
          self.nzHeight) {
...
$.fn.elevateZoom.options = {
  ...
  tintColour:
      '#333',  // default tint color, can be anything, red, #ccc, rgb(0,0,0)
  ...
  cursor: 'default',     // user should set to what they want the cursor as, if
                         // they have set a click function
  responsive: true,
  onComplete: $.noop,
