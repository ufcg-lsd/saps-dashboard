var describe = window.describe, expect = window.expect, it = window.it,
...
describe('c3 chart axis', function() {
  ...
  var args = {
    ...
    axis: {
      y: {tick: {values: null, count: undefined}},
      y2: {tick: {values: null, count: undefined}}
    }
  };
  ...
  describe('axis y timeseries', function() {
    var args = {
      data: {columns: [['times', 60000, 120000, 180000, 240000]]},
      axis: {y: {type: 'timeseries', tick: {time: {}}}}
    };
    ...
    it('should have 7 ticks on y axis', function() {
      var ticksSize = d3.select('.c3-axis-y').selectAll('g.tick').size();
      expect(ticksSize).toBe(7);  // the count starts at initial value and
                                  // increments by the set interval
    });
    ...
    it('should update args to set axis.y.time', function() {
      args.axis.y.tick.time = {value: 'seconds', interval: 60};
      expect(true).toBeTruthy();
    });

    it('should have 4 ticks on y axis', function() {
      var ticksSize = d3.select('.c3-axis-y').selectAll('g.tick').size();
      expect(ticksSize).toBe(4);  // the count starts at initial value and
                                  // increments by the set interval
    });
    ...
    describe('indexed x axis and y/y2 axis', function() {
      describe('not rotated', function() {
        ...
        it('should construct indexed x axis properly', function() {
          var ticks =
                  chart.internal.main.select('.c3-axis-x').selectAll('g.tick'),
        ...
        it('should construct y axis properly', function() {
          var ticks =
                  chart.internal.main.select('.c3-axis-y').selectAll('g.tick'),
              expectedX = '-9', expectedDy = '3';
        ...
        it('should construct y2 axis properly', function() {
          var ticks =
                  chart.internal.main.select('.c3-axis-y2').selectAll('g.tick'),
              expectedX = '9', expectedDy = '3';
          expect(ticks.size()).toBe(9);
          ticks.each(function(d) {
            var tspans = d3.select(this).selectAll('tspan');
      ...
      describe('rotated', function() {
        ...
        it('should not split y axis tick text to multiple lines', function() {
          var ticks =
              ...
              expectedTexts =
                  [
                    '0', '500000000000000', '1000000000000000',
                    '1500000000000000', '2000000000000000', '2500000000000000',
                    '3000000000000000'
                  ],
              expectedX = '0', expectedDy = '.71em';
      ...
      describe('not rotated', function() {
        it('should update args successfully', function() {
          args = {
            data: {
              ...
              columns: [
                [
                  'x', 'this is a very long tick text on category axis', 'cat1',
                  'cat2', 'cat3', 'cat4', 'cat5'
                ],
                ['data1', 30, 200, 100, 400, 150, 250],
                ['data2', 50, 20, 10, 40, 15, 25]
        ...
        it('should locate ticks properly', function() {
          var ticks =
              chart.internal.main.select('.c3-axis-x').selectAll('g.tick');
          ticks.each(function(d, i) {
            var tspans = d3.select(this).selectAll('tspan'), expectedX = '0',
                expectedDy = '.71em';
        ...
        it('should split tick text properly', function() {
          ...
          tspans.each(function(d, i) {
            ...
            expect(tspan.attr('x')).toBe(expectedX);
            // unable to define pricise number because it differs depends on
            // environment..
            if (i === 0) {
              expect(tspan.attr('dy')).toBe('.71em');
            } else {
      ...
      describe('rotated', function() {
        ...
        it('should locate ticks on rotated axis properly', function() {
          var ticks =
              chart.internal.main.select('.c3-axis-x').selectAll('g.tick');
          ticks.each(function(d, i) {
            var tspans = d3.select(this).selectAll('tspan'), expectedX = '-9',
                expectedDy = '3';
            if (i > 0) {  // i === 0 should be checked in next test
              expect(tspans.size()).toBe(1);
              tspans.each(function() {
      ...
      describe('option used', function() {
        describe('as null', function() {
          ...
          it('should split x tick', function() {
            var tick =
                    chart.internal.main.select('.c3-axis-x').select('g.tick'),
                tspans = tick.selectAll('tspan');
            expect(tspans.size()).toBe(1);
          });
        });
    ...
    describe('not rotated', function() {
      ...
      it('should rotate tick texts', function() {
        chart.internal.main.selectAll('.c3-axis-x g.tick').each(function() {
          var tick = d3.select(this), text = tick.select('text'),
      ...
      it('should have automatically calculated x axis height', function() {
        var box = chart.internal.main.select('.c3-axis-x')
                      .node()
