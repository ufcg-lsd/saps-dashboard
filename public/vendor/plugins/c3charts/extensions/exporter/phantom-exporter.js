...
// useful python-styled string formatting, "hello {0}! Javascript is
// {1}".format("world", "awesome");
...
var page = require('webpage').create(), fs = require('fs'),
    system = require('system'), config = JSON.parse(fs.read('config.json')),
    output, size;
...
if (system.args.length < 3) {
  ...
  if (system.args[3]) {
    var dimensions = system.args[3].split('x'), width = dimensions[0],
        height = dimensions[1];
    ...
    if (!opts.size) {
      opts.size = {'height': height, 'width': width};
    }
  } else {
...
page.onLoadFinished =
  ...
  // setting transition to 0 has proven not to work thus far, but 300ms isn't
  // much so this is acceptable for now
  setTimeout(function() {
    page.render(out);
    phantom.exit();
  }, 300);
