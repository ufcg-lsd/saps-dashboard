...
c3ext.generate =
  ...
  var zoom2 =
      c3ext.ZoomBehavior({changed: onZoomChanged, bindto: options.bindto});
  ...
  function zoomAndReduceData(list, zoomRange, func, maxItems) {
    ...
    // console.log("x" + getCurrentZoomLevel() + ", maxItems=" + maxItems + "
    // chunkSize=" + chunkSize + " totalBefore=" + list2.length + ", totalAfter="
    // + list3.length);
    return list3;
  }
  ...
  var getDataForZoom = function(data) {
    ...
    data.columns = originalData.columns.map(function(column) {
      ...
      var newValues = zoomAndReduceData(
          values, zoomInfo.currentZoom, reducer, _zoom2_maxItems);
  ...
  zoom.getZoom =
    ...
    applyZoomAndPan();
              ...
              zoom.zoomAndPanByRatio =
                  function(zoomRatio, panRatio) {
    var pageSize = getItemsToShow();
  ...
  function
  element_mousewheel(e) {
    deltaY += e.deltaY;
    leftRatio = (e.offsetX - 70) / (e.currentTarget.offsetWidth - 70);
    // console.log({ "e.offsetX": e.offsetX, "e.currentTarget.offsetWidth":
    // e.currentTarget.offsetWidth, leftRatio: leftRatio });
