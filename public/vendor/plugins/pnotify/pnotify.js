...
$.extend(PNotify.prototype, {
  ...
  state: 'initializing',  // The state can be "initializing", "opening", "open",
                          // "closing", and "closed".
  ...
  init: function() {
    ...
    // First and foremost, we don't want our module objects all referencing the
    // prototype.
    this.modules = {};
    ...
    if (this.options.cornerclass !== '')
      this.container.removeClass('ui-corner-all')
          ...
          .append($('<span />', {
            'class': this.options.icon === true ?
                (this.options.type === 'error' ?
                     ...
                     (this.options.type === 'primary' ?
                          this.styles.primary_icon :
                          (this.options.type === 'info' ?
                               this.styles.info_icon :
                               (this.options.type === 'success' ?
                                    this.styles.success_icon :
                                    (this.options.type === 'warning' ?
                                         this.styles.warning_icon :
                                         (this.options.type === 'danger' ?
                                              this.styles.danger_icon :
                                              (this.options.type === 'alert' ?
                                                   this.styles.alert_icon :
                                                   (this.options.type ===
                                                            'system' ?
                                                        this.styles
                                                            .system_icon :
    ...
    this.title_container =
        $('<h4 />', {'class': 'ui-pnotify-title'}).appendTo(this.container);
    ...
    // Add text.
    this.text_container =
        $('<div />', {'class': 'ui-pnotify-text'}).appendTo(this.container);
  ...
  update: function(options) {
    ...
    if (this.options.type !== oldOpts.type)
      this.container
          .removeClass(
              this.styles.error + ' ' + this.styles.notice + ' ' +
              this.styles.primary + ' ' + this.styles.info + ' ' +
              this.styles.success + ' ' + this.styles.warning + ' ' +
              this.styles.danger + ' ' + this.styles.alert + ' ' +
              this.styles.system + ' ' + this.styles.dark)
          .addClass(
              this.options.type === 'error' ?
                  this.styles.error :
                  (this.options.type === 'primary' ?
  ...
  open: function() {
    ...
    if (!this.elem.parent().length)
      this.elem.appendTo(
          this.options.stack.context ? this.options.stack.context : body);
    // Try to put it in the right position.
    if (this.options.stack.push !== 'top') this.position(true);
    // First show it, then set its opacity, then hide it.
    if (this.options.animation === 'fade' ||
        this.options.animation.effect_in === 'fade') {
  ...
  animateIn: function(callback) {
    ...
    if (animation === 'none') {
      ...
      this.elem.show().fadeTo(
          this.options.animate_speed, this.options.opacity, callback);
      ...
      this.elem.show(
          animation,
          (typeof this.options.animation.options_in === 'object' ?
               this.options.animation.options_in :
               {}),
          this.options.animate_speed, callback);
  ...
  position: function(dontSkipHidden) {
    ...
    if (e.parent().hasClass('ui-effects-wrapper'))
      e = this.elem.css({'left': '0', 'top': '0', 'right': '0', 'bottom': '0'})
              .parent();
    ...
    if (!hidden || dontSkipHidden) {
      ...
      if ((s.dir1 === 'down' &&
           s.nextpos1 + e.height() > (s.context.is(body) ?
                                          jwindow.height() :
          ...
          (s.dir1 === 'up' &&
           s.nextpos1 + e.height() > (s.context.is(body) ?
                                          ...
                                          s.context.prop('scrollHeight'))) ||
          (s.dir1 === 'left' &&
           s.nextpos1 + e.width() > (s.context.is(body) ?
                                         jwindow.width() :
        ...
        s.nextpos1 = s.firstpos1;
        s.nextpos2 +=
            s.addpos2 + (typeof s.spacing2 === 'undefined' ? 25 : s.spacing2);
        s.addpos2 = 0;
      }
      ...
      // Keep track of the widest/tallest notice in the column/row, so we can
      // push the next column/row.
      switch (s.dir2) {
        case 'down':
        case 'up':
      ...
      if (typeof s.nextpos1 === 'number') {
        ...
        if (s.animation &&
            (curpos1 > s.nextpos1 || animate.top || animate.bottom ||
      ...
      if (animate.top || animate.bottom || animate.right || animate.left)
        e.animate(
            animate,
            {duration: this.options.position_animate_speed, queue: false});
      // Calculate the next dir1 position.
      switch (s.dir1) {
        case 'down':
        case 'up':
          s.nextpos1 += e.height() +
              (typeof s.spacing1 === 'undefined' ? 25 : s.spacing1);
          break;
        case 'left':
        case 'right':
          s.nextpos1 +=
              e.width() + (typeof s.spacing1 === 'undefined' ? 25 : s.spacing1);
          break;
      }
    }
    return this;
  ...
  cancelRemove: function() {
    ...
    if (this.state === 'closing') {
      ...
      this.elem.css('height', 'auto')
          .animate(
              {'width': this.options.width, 'opacity': this.options.opacity},
              'fast');
    }
    return this;
  },
...
PNotify.prototype.modules.buttons = {
  ...
  init: function(notice, options) {
    ...
    notice.elem.on({
      'mouseenter': function(e) {
        // Show the buttons.
        if (that.myOptions.sticker &&
            !(notice.options.nonblock && notice.options.nonblock.nonblock))
          that.sticker.trigger('pnotify_icon').css('visibility', 'visible');
        if (that.myOptions.closer &&
            !(notice.options.nonblock && notice.options.nonblock.nonblock))
            ...
            .bind(
                ...
                function() {
                  $(this)
                      .children()
                      .removeClass(
                          notice.styles.pin_up + ' ' + notice.styles.pin_down)
                      .addClass(
                          notice.options.hide ? notice.styles.pin_up :
                                                notice.styles.pin_down);
                })
            .append($(
                '<span />',
                {'class': notice.styles.pin_up, 'title': options.labels.stick}))
    ...
    this.closer =
        $('<div />', {
          ...
          'click': function() {
            ...
            .append($(
                '<span />',
                {'class': notice.styles.closer, 'title': options.labels.close}))
            .prependTo(notice.container);
    if (!options.closer ||
        (notice.options.nonblock && notice.options.nonblock.nonblock))
...
$.extend(
    PNotify.styling.bootstrap2,
    {closer: 'icon-remove', pin_up: 'icon-pause', pin_down: 'icon-play'});
...
$.extend(
    PNotify.styling.fontawesome,
    {closer: 'fa fa-times', pin_up: 'fa fa-pause', pin_down: 'fa fa-play'});
}));
// Confirm
// Uses AMD or browser globals for jQuery.
(function(factory) {
  ...
  buttons: [
    {
      text: 'Ok',
      addClass: '',
      // Whether to trigger this button when the user hits enter in a single
      // line prompt.
      promptTrigger: true,
...
PNotify.prototype.modules.confirm = {
  ...
  makeDialog: function(notice, options) {
    ...
    if (options.prompt) {
      this.prompt =
          $('<' +
            (options.prompt_multi_line ? 'textarea rows="5"' :
                                         'input type="text"') +
              ...
              .on('click', (function(btn) {
                    return function() {
                      if (typeof btn.click == 'function') {
                        btn.click(
                            notice, options.prompt ? that.prompt.val() : null);
                      }
                    }
                  })(btn));
...
$.extend(PNotify.styling.bootstrap2, {btn: 'btn', input: ''});
$.extend(
    PNotify.styling.bootstrap3,
    {btn: 'btn btn-default', input: 'form-control'});
$.extend(
    PNotify.styling.fontawesome,
    {btn: 'btn btn-default', input: 'form-control'});
}));
...
PNotify.prototype.options.desktop = {
  ...
  // notices between tabs.
  // If you leave tag null, one will be generated, facilitating the "update"
  // function.
...
PNotify.prototype.modules.desktop = {
  ...
  genNotice: function(notice, options) {
    if (options.icon === null) {
      this.icon = 'http://sciactive.com/pnotify/includes/desktop/' +
          notice.options.type + '.png';
    ...
    notice.desktop = notify(
        notice.options.title,
        {icon: this.icon, body: notice.options.text, tag: this.tag});
