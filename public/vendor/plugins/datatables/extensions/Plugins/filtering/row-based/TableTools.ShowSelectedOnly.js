...
$.fn.dataTable.SelectedLengthMenu = function(oSettings) {
  ...
  /* This can be overruled by not using the _MENU_ var/macro in the language
   * variable */
  ...
  if (aLengthMenu.length == 2 && typeof aLengthMenu[0] === 'object' &&
    ...
    for (i = 0, iLen = aLengthMenu.length; i < iLen; i++) {
      sStdMenu += '<option value="' + aLengthMenu[i] + '">' + aLengthMenu[i] +
          '</option>';
    }
  }
  sStdMenu += '</select>';
  ...
  if (typeof oFilterSelectedOptions === 'object') {
    ...
    sSelectedMenu += '<option value="Selected">' +
        oFilterSelectedOptions.SelectedText + '</option>';
  } else {
    sSelectedMenu +=
  ...
  nLength.innerHTML = '<span>' +
      sLengthMenu.replace('_MENU_', sStdMenu)
          .replace('_SELECTEDMENU_', sSelectedMenu) +
      '</span>';

  var $lengthSelect =
      $('select[name="' + oSettings.sTableId + '_length"]', nLength);
  if ($lengthSelect.length == 0) $lengthSelect = $('select :eq(0)', nLength);

  /*
   * Set the length to the current display length - thanks to Andrea Pavlovic
   * for this fix, and Stefan Skopnik for fixing the fix!
   */
  $lengthSelect
  ...
  var $filterSelectedSelect =
      $('select[name="' + oSettings.sTableId + '_selectedFilter"]', nLength);
