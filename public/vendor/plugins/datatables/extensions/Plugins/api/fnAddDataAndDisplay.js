...
jQuery.fn.dataTableExt.oApi.fnAddDataAndDisplay = function(oSettings, aData) {
  ...
  /* Need to re-filter and re-sort the table to get positioning correct, not
   * perfect as this will actually redraw the table on screen, but the update
   * should be so fast (and possibly not alter what is already on display) that
