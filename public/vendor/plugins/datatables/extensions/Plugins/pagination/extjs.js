...
$.fn.dataTableExt.oPagination.extStyle = {
  ...
  'fnInit': function(oSettings, nPaging, fnCallbackDraw) {
    ...
    nPageNumBox =
        $('<input />', {type: 'text', val: 1, 'class': 'pageinate_input_box'});
    nPageOf = $('<span />', {text: '/'});
    nTotalPages =
        $('<span />', {class: 'paginate_total', text: oPaging.iTotalPages});
    ...
    nPageNumBox.change(function() {
      var pageValue = parseInt($(this).val(), 10) -
          1;  // -1 because pages are 0 indexed, but the UI is 1
  ...
  'fnUpdate': function(oSettings, fnCallbackDraw) {
    ...
    $(an).each(function(index, item) {
      ...
      if (oPaging.iPage == 0) {
        var prev = $item.find('span.paginate_button.first')
      ...
      if (oPaging.iPage + 1 == oPaging.iTotalPages) {
        var next = $item.find('span.paginate_button.last')
