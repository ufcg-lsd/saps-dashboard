  ...
  var k = function(a, b) {
    ...
    if (!this instanceof k)
      ...
      d.fn.dataTable.camelToHungarian &&
          d.fn.dataTable.camelToHungarian(k.defaults, b);
      var g = d.fn.dataTable.Api ? (new d.fn.dataTable.Api(a)).settings()[0] :
                                   a.fnSettings();
      this.s = {
        dt: g,
      ...
      g._bInitComplete ? this._fnConstruct(b) :
                         g.oApi._fnCallbackReg(g, 'aoInitComplete', function() {
                           c._fnConstruct(b)
                         }, 'FixedColumns')
    }
  };
  k.prototype = {
    ...
    _fnConstruct: function(a) {
      ...
      if ('function' != typeof this.s.dt.oInstance.fnVersionCheck ||
        ...
        this.s = d.extend(!0, this.s, k.defaults, a);
        ...
        var g = 'onwheel' in r.createElement('div') ? 'wheel.DTFC' :
                                                      'mousewheel.DTFC';
        if (0 < b.s.iLeftColumns)
          d(b.dom.grid.left.liner)
              ...
              .on(g, function(a) {
                b.dom.scroller.scrollLeft -= 'wheel' === a.type ?
    ...
    _fnGridSetup: function() {
      ...
      0 < this.s.iRightColumns &&
          (this.dom.grid.right.wrapper = f,
           ...
           b.style.width = a.bar + 'px', b.style.right = -a.bar + 'px',
           this.dom.grid.right.footBlock = b, c.appendChild(f));
      if (this.s.dt.nTFoot &&
    ...
    _fnGridLayout: function() {
      ...
      0 < this.s.iLeftColumns &&
          (a.left.wrapper.style.width = e + 'px',
           ...
           a.left.liner.style.width = e + f.bar + 'px',
           a.left.liner.style.height = c + 'px');
      0 < this.s.iRightColumns &&
          (b -= h, f.y && (b -= f.bar), a.right.wrapper.style.width = h + 'px',
           ...
           a.right.liner.style.width = h + f.bar + 'px',
      ...
      for (var c = [], g = [], f = [], e = 0, h = a.length; e < h; e++) {
        ...
        for (var j = 0, n = this.s.iTableColumns; j < n; j++)
          if (-1 !== d.inArray(j, b)) {
            ...
            -1 === l ? (l = d(a[e][j].cell).clone(!0, !0)[0], g.push(l),
                        f.push(a[e][j].cell),
                        i.push({cell: l, unique: a[e][j].unique})) :
    ...
    _fnClone: function(a, b, c, g) {
      ...
      if (g) {
        ...
        for (h = m.length; e < h; e++) {
          ...
          for (j = m[e].length; i < j; i++)
            p[e][i].cell.className = m[e][i].cell.className,
            d('span.DataTables_sort_icon', p[e][i].cell).each(function() {
              this.className =
                  d('span.DataTables_sort_icon', m[e][i].cell)[0].className
            })
        }
      }
      ...
      if (0 < this.s.dt.aiDisplay.length) {
        ...
        for (k = 0; k < c.length; k++)
          n = c[k], l = d(this.s.dt.aoColumns[n].nTh).clone(!0)[0],
          l.innerHTML = '', j = l.style, j.paddingTop = '0',
          j.paddingBottom = '0', j.borderTopWidth = '0',
          j.borderBottomWidth = '0', j.height = 0,
          j.width = f.s.aiInnerWidths[n] + 'px', h.appendChild(l);
        d('>tbody>tr', f.dom.body).each(function(a) {
          var b = this.cloneNode(false);
          b.removeAttribute('id');
          a = f.s.dt.oFeatures.bServerSide === false ?
      ...
      g && 'undefined' != typeof this.s.dt.oScroller &&
          b.liner.appendChild(this.s.dt.oScroller.dom.force.cloneNode(!0));
      ...
      if (null !== this.s.dt.nTFoot) {
        if (g) {
          ...
          m = this._fnCopyLayout(this.s.dt.aoFooter, c);
          b = [];
          this.s.dt.oApi._fnDetectHeader(b, d('>tfoot', a.footer)[0]);
          e = 0;
          for (h = m.length; e < h; e++) {
            i = 0;
            for (j = m[e].length; i < j; i++)
      ...
      null !== f.s.dt.nTFoot &&
          (b = this.s.dt.oApi._fnGetUniqueThs(
               this.s.dt, d('>tfoot', a.footer)[0]),
           d(b).each(function(a) {
             n = c[a];
             this.style.width = f.s.aiInnerWidths[n] + 'px'
           }))
    ...
    _fnEqualiseHeights: function(a, b, c) {
      if (!('none' == this.s.sHeightMatch && 'thead' !== a && 'tfoot' !== a)) {
        var g, f, e = b.getElementsByTagName(a)[0],
                  c = c.getElementsByTagName(a)[0],
                  a = d('>' + a + '>tr:eq(0)', b).children(':first');
        a.outerHeight();
        a.height();
        for (var e = this._fnGetTrNodes(e), b = this._fnGetTrNodes(c), h = [],
                 c = 0, a = b.length;
             c < a; c++)
          g = e[c].offsetHeight, f = b[c].offsetHeight, g = f > g ? f : g,
