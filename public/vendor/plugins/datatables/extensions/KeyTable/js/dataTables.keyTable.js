...
var factory = function($, DataTable) {
  ...
  KeyTable = function(oInit) {
     ...
     * Scope:    KeyTable - public
     * Notes:    This object contains all the public methods for adding and
     * removing events - these are dynamically added later on
     */
    this.event = {'remove': {}};
    ...
    /*
     * Variable: _oaoEvents
     * Purpose:  Event cache object, one array for each supported event for
     * speed of searching Scope:    KeyTable - private
     */
    var _oaoEvents = {'action': [], 'esc': [], 'focus': [], 'blur': []};
    ...
    function _fnEventAddTemplate(sKey) {
       ...
       *         or
       *           1. int:x - x coord. of target cell (can be null for live
       * events)
       *           2. int:y - y coord. of target cell (can be null for live
       * events)
       *           3. function:z - callback function to apply
       * Notes:    This function is (interally) overloaded (in as much as
       * javascript allows for that) - the target cell can be given by either
       * node or coords.
       */
      return function(x, y, z) {
        if ((x === null || typeof x == 'number') &&
            (y === null || typeof y == 'number') && typeof z == 'function') {
          ...
          alert(
              'Unhandable event type was added: x' + x + '  y:' + y +
              '  z:' + z);
        }
      };
    }
    ...
    function _fnEventRemoveTemplate(sKey) {
       ...
       * Notes:    This function is (interally) overloaded (in as much as
       * javascript allows for that) - the target cell can be given by either
       * node or coords and the function to remove is optional
       */
      return function(x, y, z) {
        if ((x === null || typeof arguments[0] == 'number') &&
          ...
          alert(
              'Unhandable event type was removed: x' + x + '  y:' + y +
              '  z:' + z);
        }
      };
    }
     ...
     * Returns:  -
     * Inputs:   string:sType - type of event to add, given by the available
     * elements in _oaoEvents int:x - x-coords to add event to - can be null for
     * "blanket" event int:y - y-coords to add event to - can be null for
     * "blanket" event function:fn - callback function for when triggered
     */
     ...
     *           function:fn - optional - remove this function. If not given all
     * handlers of this type will be removed
     */
    function _fnEventRemove(sType, x, y, fn) {
      var iCorrector = 0;

      for (var i = 0, iLen = _oaoEvents[sType].length; i < iLen - iCorrector;
           i++) {
     ...
     * Purpose:  Look thought the events cache and fire off the event of
     * interest Returns:  int:iFired - number of events fired Inputs:
     * string:sType - type of event to look for int:x - x coord of cell int:y -
     * y coord of  ell Notes:    It might be more efficient to return after the
     * first event has been tirggered, but that would mean that only one
     * function of a particular type can be subscribed to a particular node.
     */
     ...
     *           bool:bAutoScroll - optional - should we scroll the view port to
     * the display
     */
    function _fnSetFocus(nTarget, bAutoScroll) {
      ...
      /* If it's a DataTable then we need to jump the paging to the relevant
       * page */
      var oSettings;
      if (_oDatatable) {
      ...
      var iViewportHeight, iViewportWidth, iScrollTop, iScrollLeft, iHeight,
          iWidth, aiPos;
      if (bAutoScroll) {
        ...
        /* Take account of scrolling in DataTables 1.7 - remove scrolling since
         * that would add to the positioning calculation
         */
     ...
     * Purpose:  Remove focus from a cell and fire any blur events which are
     * attached Returns:  - Inputs:   node:nTarget - cell of interest
     */
     ...
     */
    function _fnKey(e) {
      /* If user or system has blocked KeyTable from doing anything, just ignore
       * this event */
      ...
      var x, y,
          iTableWidth = _nBody.getElementsByTagName('tr')[0]
                            .getElementsByTagName('td')
                            .length,
          iTableHeight;

      /* Get table height and width - done here so as to be dynamic (if table is
       * updated) */
      if (_oDatatable) {
        /*
         * Locate the current node in the DataTable overriding the old positions
         * - the reason for is is that there might have been some DataTables
         * interaction between the last focus and now
         */
        iTableHeight = _oDatatable.aiDisplay.length;

        var aDtPos = _fnFindDtCell(_nOldFocus);
        if (aDtPos === null) {
          /* If the table has been updated such that the focused cell can't be
           * seen - do nothing */
          return;
        }
        _iOldX = aDtPos[0];
      ...
      switch (iKey) {
        ...
        case 37: /* left arrow */
          if (_iOldX > 0) {
            ...
            /* at start of table */
            if (iKey == -1 && _bForm) {
              /* If we are in a form, return focus to the 'input' element such
               * that tabbing will follow correctly in the browser
               */
              _bInputFocused = true;
              _nInput.focus();

              /* This timeout is a little nasty - but IE appears to have some
               * asyhnc behaviour for focus
               */
              setTimeout(function() {
    ...
    function _fnCellFromCoords(x, y) {
      if (_oDatatable) {
        if (typeof _oDatatable.aoData[_oDatatable.aiDisplay[y]] !=
            'undefined') {
          return _oDatatable.aoData[_oDatatable.aiDisplay[y]]
     ...
     * Notes:    Not actually interested in this for DataTables since it might
     * go out of date
     */
     ...
     * Notes:    This is so nasty, but without browser detection you can't tell
     * which you should set So on browsers that support both, the scroll top
     * will be set twice. I can live with that :-)
     */
     ...
     * Purpose:  Get the coords. of a cell from the DataTables internal
     * information Returns:  array[2] int: [x, y] coords. or null if not found
     ...
     * Returns:  -
     * Inputs:   object:oInit - optional - Initalisation object with the
     * following parameters: array[2] int:focus - x and y coordinates of the
     * initial target or node:focus - the node to set initial focus on
     *   node:table - the table to use, if not given, first table with class
     * 'KeyTable' will be used string:focusClass - focusing class to give to
     * table elements object:that - focus bool:initScroll - scroll the view port
     * on load, default true int:tabIndex - the tab index to give the hidden
     * input element
     */
    function _fnInit(table, datatable, oInit, that) {
      ...
      /* If the table is inside a form, then we need a hidden input box which
       * can be used by the browser to catch the browser tabbing for our table
       */
      if (_bForm) {
