...
var factory = function($, DataTable) {
  ...
  var AutoFill = function(oDT, oConfig) {
    ...
    this.s = {
      /**
       * @namespace Cached information about the little dragging icon (the
       * filler)
       */
      ...
      /**
       * @namespace Data cache for information that we need for scrolling the
       * screen when we near the edges
       */
      'screen': {'interval': null, 'y': 0, 'height': 0, 'scrollTop': 0},

      /**
       * @namespace Data cache for the position of the DataTables scrolling
       * element (when scrolling is enabled)
       */
  ...
  AutoFill.prototype = {
    ...
    '_fnInit': function(dt, config) {
      ...
      // fnSettings method
      this.s.dt =
          DataTable.Api ? new DataTable.Api(dt).settings()[0] : dt.fnSettings();
      this.s.init = config || {};
      ...
      var filler = $('<div/>', {'class': 'AutoFill_filler'}).appendTo('body');
      ...
      border = $('<div/>', {'class': 'AutoFill_border'});
      ...
      $('tbody', this.dom.table)
          .on('mouseover.DTAF mouseout.DTAF', '>tr>td, >tr>th', function(e) {
    ...
    _initColumns: function() {
      ...
      dt.oApi._fnApplyColumnDefs(
          dt, config.aoColumnDefs || config.columnDefs,
          config.aoColumns || config.columns, function(colIdx, def) {
    ...
    '_fnUpdateBorder': function(nStart, nEnd) {
      var border = this.s.border.width, offsetStart = $(nStart).offset(),
          offsetEnd = $(nEnd).offset(), x1 = offsetStart.left - border,
          x2 = offsetEnd.left + $(nEnd).outerWidth(),
          y1 = offsetStart.top - border,
          y2 = offsetEnd.top + $(nEnd).outerHeight(),
          width = offsetEnd.left + $(nEnd).outerWidth() - offsetStart.left +
          (2 * border),
          height = offsetEnd.top + $(nEnd).outerHeight() - offsetStart.top +
          (2 * border),
          oStyle;
      ...
      if (offsetStart.left > offsetEnd.left) {
        ...
        width = offsetStart.left + $(nStart).outerWidth() - offsetEnd.left +
            (2 * border);
      }

      if (this.s.dt.oScroll.sY !== '') {
        /* The border elements are inside the DT scroller - so position relative
         * to that */
    ...
    '_fnFillerDragStart': function(e) {
      ...
      /* Scrolling handler - we set an interval (which is cancelled on mouse up)
       * which will fire regularly and see if we need to do any scrolling
       */
      this.s.screen.interval = setInterval(function() {
        ...
        if (that.s.screen.height - that.s.screen.y + iScrollTop < 50) {
          $('html, body')
              .animate({'scrollTop': iScrollTop + 50}, 240, 'linear');
    ...
    /**
     * Mouse move event handler for during a move. See if we want to update the
     * display based on the new cursor position
    ...
    /**
     * Mouse release handler - end the drag and take action to update the cells
    ...
    '_fnFillerFinish': function(e) {
      ...
      for (i = 0, iLen = cells.length; i < iLen; i++) {
        ...
        var stepValue = column.step.call(
            column, cell.node, read, previous, i, cell.x, cell.y);
    ...
    '_fnFillerDisplay': function(e) {
      ...
      var nTd = (e.target.nodeName.toLowerCase() == 'td') ?
          e.target :
      ...
      if (e.type == 'mouseover') {
        ...
        filler.style.display = 'block';
          ...
          !e.relatedTarget || !e.relatedTarget.className.match(/AutoFill/)) {
        filler.style.display = 'none';
      }
    },
    ...
    '_fnFillerPosition': function(nTd) {
      ...
      var filler = this.dom.filler;
      filler.style.top =
          (offset.top - (this.s.filler.height / 2) - 1 + $(nTd).outerHeight()) +
          'px';
      filler.style.left =
          (offset.left - (this.s.filler.width / 2) - 1 + $(nTd).outerWidth()) +
          'px';
    }
  };
  ...
  AutoFill.defaults = {
    /**
     * Mode for dragging (restrict to y-axis only, x-axis only, either one or
     * none):
     *
    ...
    column: {
      ...
      step: function(cell, read, last, i, x, y) {
        ...
        if (match) {
          return last.replace(
              re, parseInt(match[1], 10) + (x < 0 || y < 0 ? -1 : 1));
        }
