...
var factory = function($, DataTable) {
   ...
   *    1.10 this can also be a jQuery collection (with just a single table in
   * its result set), a jQuery selector, DataTables API instance or settings
   *    object.
  ...
  FixedHeader = function(mTable, oInit) {
    ...
    var oSettings = {
      'aoCache': [],
      'oSides': {'top': true, 'bottom': false, 'left': 0, 'right': 0},
    ...
    var dt = $.fn.dataTable.Api ? new $.fn.dataTable.Api(mTable).settings()[0] :
                                  mTable.fnSettings();
  ...
  FixedHeader.prototype = {
    ...
    fnInit: function(oDtSettings, oInit) {
      ...
      if (s.oSides.top) {
        s.aoCache.push(that._fnCloneTable(
            'fixedHeader', 'FixedHeader_Header', that._fnCloneThead));
      }
      if (s.oSides.bottom) {
        s.aoCache.push(that._fnCloneTable(
            'fixedFooter', 'FixedHeader_Footer', that._fnCloneTfoot));
      }
      if (s.oSides.left) {
        s.aoCache.push(that._fnCloneTable(
            'fixedLeft', 'FixedHeader_Left', that._fnCloneTLeft,
            s.oSides.left));
      }
      if (s.oSides.right) {
        s.aoCache.push(that._fnCloneTable(
            'fixedRight', 'FixedHeader_Right', that._fnCloneTRight,
            s.oSides.right));
      }
    ...
    _fnCloneTable: function(sType, sClass, fnClone, iCells) {
      ...
      /* We know that the table _MUST_ has a DIV wrapped around it, because this
       * is simply how DataTables works. Therefore, we can set this to be
       * relatively position (if it is not alreadu absolute, and use this as the
       * base point for the cloned header
       */
     ...
     */
    _fnMeasure: function() {
      var s = this.fnGetSettings(), m = s.oMes, jqTable = $(s.nTable),
          oOffset = jqTable.offset(),
          iParentScrollTop =
              this._fnSumScroll(s.nTable.parentNode, 'scrollTop'),
          iParentScrollLeft =
              this._fnSumScroll(s.nTable.parentNode, 'scrollLeft');
     ...
     * Purpose:  Loop over the fixed elements for this table and update their
     * positions Returns:  - Inputs:   -
     */
     ...
     * Purpose:  Loop over the fixed elements for this table and call their
     * cloning functions Returns:  - Inputs:   -
     */
    ...
    _fnScrollHorizontalRight: function(oCache) {
      var s = this.fnGetSettings(), oMes = s.oMes, oWin = FixedHeader.oWin,
          oDoc = FixedHeader.oDoc, nTable = oCache.nWrapper,
          iFixedWidth = $(nTable).outerWidth();

      if (oWin.iScrollRight < oMes.iTableRight) {
        /* Fully right aligned */
        this._fnUpdateCache(
            oCache, 'sPosition', 'absolute', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', oMes.iTableTop + 'px', 'top', nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft',
            (oMes.iTableLeft + oMes.iTableWidth - iFixedWidth) + 'px', 'left',
            nTable.style);
      } else if (
          oMes.iTableLeft < oDoc.iWidth - oWin.iScrollRight - iFixedWidth) {
        /* Middle */
        this._fnUpdateCache(
            oCache, 'sPosition', 'fixed', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', (oMes.iTableTop - oWin.iScrollTop) + 'px', 'top',
            nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', (oWin.iWidth - iFixedWidth) + 'px', 'left',
            nTable.style);
      } else {
        /* Fully left aligned */
        this._fnUpdateCache(
        ...
        this._fnUpdateCache(
            oCache, 'sTop', oMes.iTableTop + 'px', 'top', nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', oMes.iTableLeft + 'px', 'left', nTable.style);
      }
    },
    ...
    _fnScrollHorizontalLeft: function(oCache) {
      ...
      if (oWin.iScrollLeft < oMes.iTableLeft) {
        ...
        this._fnUpdateCache(
          ...
          oWin.iScrollLeft < oMes.iTableLeft + oMes.iTableWidth - iCellWidth) {
        this._fnUpdateCache(
            oCache, 'sPosition', 'fixed', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', (oMes.iTableTop - oWin.iScrollTop) + 'px', 'top',
        ...
        this._fnUpdateCache(
            oCache, 'sLeft',
            (oMes.iTableLeft + oMes.iTableWidth - iCellWidth) + 'px', 'left',
            nTable.style);
      }
    },
    ...
    _fnScrollFixedFooter: function(oCache) {
      ...
      if (oWin.iScrollBottom < oMes.iTableBottom) {
        ...
        this._fnUpdateCache(
            oCache, 'sTop',
            (oMes.iTableTop + oMes.iTableHeight - iCellHeight) + 'px', 'top',
            nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', oMes.iTableLeft + 'px', 'left', nTable.style);
      } else if (
        ...
        this._fnUpdateCache(
            oCache, 'sPosition', 'fixed', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', (oWin.iHeight - iCellHeight) + 'px', 'top',
            nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', (oMes.iTableLeft - oWin.iScrollLeft) + 'px',
            'left', nTable.style);
      } else {
        /* Above */
        this._fnUpdateCache(
            oCache, 'sPosition', 'absolute', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', (oMes.iTableTop + iCellHeight) + 'px', 'top',
            nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', oMes.iTableLeft + 'px', 'left', nTable.style);
      }
    ...
    _fnScrollFixedHeader: function(oCache) {
      var s = this.fnGetSettings(), oMes = s.oMes, oWin = FixedHeader.oWin,
          oDoc = FixedHeader.oDoc, nTable = oCache.nWrapper, iTbodyHeight = 0,
      ...
      if (oMes.iTableTop > oWin.iScrollTop + s.oOffset.top) {
        ...
        this._fnUpdateCache(
          ...
          oWin.iScrollTop + s.oOffset.top > oMes.iTableTop + iTbodyHeight) {
        /* At the bottom of the table */
        this._fnUpdateCache(
            oCache, 'sPosition', 'absolute', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', (oMes.iTableTop + iTbodyHeight) + 'px', 'top',
            nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', oMes.iTableLeft + 'px', 'left', nTable.style);
      } else {
        ...
        this._fnUpdateCache(
            oCache, 'sPosition', 'fixed', 'position', nTable.style);
        this._fnUpdateCache(
            oCache, 'sTop', s.oOffset.top + 'px', 'top', nTable.style);
        this._fnUpdateCache(
            oCache, 'sLeft', (oMes.iTableLeft - oWin.iScrollLeft) + 'px',
            'left', nTable.style);
      }
    ...
    /**
     * Copy the classes of all child nodes from one element to another. This
     * implies that the two have identical structure - no error checking is
     * performed to that fact.
    ...
    _fnCloneThead: function(oCache) {
      ...
      // clicked to sort
      $('th.sorting, th.sorting_desc, th.sorting_asc', nTable)
          .bind('click', function() {
            this.blur();
          });
    },
    ...
    _fnCloneTRight: function(oCache) {
      ...
      for (var i = 0; i < oCache.iCells; i++) {
        iWidth +=
            $('thead tr th:eq(' + (iCols - 1 - i) + ')', s.nTable).outerWidth();
      }
      nTable.style.width = iWidth + 'px';
      oCache.nWrapper.style.width = iWidth + 'px';
    },
     ...
     * way. Note that this is more or less lifted as is from FixedColumns
  ...
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
   * * * * * * * * * * Static properties and methods We use these for speed!
   * This information is common to all instances of FixedHeader, so no point if
  ...
  /*
   * Variable: afnScroll
   * Purpose:  Array of functions that are to be used for the scrolling
   * components Scope:    FixedHeader
   */
  ...
  FixedHeader.fnMeasure = function() {
    var jqWin = $(window), jqDoc = $(document), oWin = FixedHeader.oWin,
  ...
  /*
   * Just one 'scroll' event handler in FixedHeader, which calls the required
   * components. This is done as an optimisation, to reduce calculation and
   * proagation time
   */
