...
XRegExp = XRegExp || function(n) {
  ...
  function o(n, r, u) {
    ...
    return u && (f = i.replace.call(f, new RegExp('[' + u + ']+', 'g'), '')),
           n = n.xregexp && !n.xregexp.isNative ?
               v(t(n.source, f),
                 n.xregexp.captureNames ? n.xregexp.captureNames.slice(0) :
                                          null) :
  ...
  function d(n) {
    return n = n || {},
           n === 'all' || n.all ? n = {natives: !0, extensibility: !0} :
                                  s(n, 'string') &&
                   (n = t.forEach(
                        n, /[^\s,]+/,
                        function(n) {
      ...
      while (o--)
        if (f = p[o],
            (f.scope === 'all' || f.scope === i) &&
                ...
                (f.pattern.lastIndex = t, e = r.exec.call(f.pattern, n),
                 e && e.index === t)) {
         ...
         r.exec =
           ...
           if (this.global || (o = this.lastIndex),
             ...
             if (!ft && r.length > 1 && a(r, '') > -1 &&
                     (e = new RegExp(
                          this.source, i.replace.call(g(this), 'g', '')),
                      i.replace.call(
                          String(t).slice(r.index), e,
                          function() {
                  ...
                  s(r, 'function') ?
                      f = i.replace.call(
                          ...
                          function() {
                            var t = arguments, i;
                            if (u)
                              for (t[0] = new String(t[0]), i = 0; i < u.length;
                                   ++i)
                                u[i] && (t[0][u[i]] = t[i + 1]);
                            return e && n.global &&
                                       (n.lastIndex =
...
if (typeof (SyntaxHighlighter) ==
    'undefined') var SyntaxHighlighter = function() {
  // CommonJS
  ...
  var sh = {
    defaults: {
       ...
       * leading zeroes. [int] - length up to which pad line numbers.
       */
    ...
    toolbar: {
      ...
      getHtml: function(highlighter) {
        var html = '<div class="toolbar">', items = sh.toolbar.items,
            list = items.list;

        function defaultGetHtml(highlighter, name) {
          return sh.toolbar.getButtonHtml(
              highlighter, name, sh.config.strings[name]);
        }

        for (var i = 0, l = list.length; i < l; i++) {
          html +=
              (items[list[i]].getHtml || defaultGetHtml)(highlighter, list[i]);
        }
      ...
      getButtonHtml: function(highlighter, commandName, label) {
        return '<span><a href="#" class="toolbar_item' +
            ' command_' + commandName + ' ' + commandName + '">' + label +
            '</a></span>';
      },
      ...
      items: {
        // Ordered lis of items in the toolbar. Can't expect `for (var n in
     ...
     *                                  parameters. Only used if element is
     * specified.
     *
    ...
    highlight: function(globalParams, element) {
      var elements = this.findElements(globalParams, element),
          propertyName = 'innerHTML', highlighter = null, conf = sh.config;

      if (elements.length === 0) return;

      for (var i = 0, l = elements.length; i < l; i++) {
        var element = elements[i], target = element.target,
            params = element.params, brushName = params.brush, code;
  ...
  function toArray(source) {
    ...
    for (var i = 0, l = source.length; i < l; i++) result.push(source[i]);
  ...
  function findElement(target, search, reverse /* optional */) {
    ...
    var nodes = reverse != true ? target.childNodes : [target.parentNode],
        propertyToFind = {'#': 'id', '.': 'className'}[search.substr(0, 1)] ||
        'nodeName',
        expectedValue, found;
  ...
  /**
   * Executes a callback on each line and replaces each line with result from
   * the callback.
  ...
  function parseParams(str) {
    var match, result = {}, arrayRegex = XRegExp('^\\[(?<values>(.*?))\\]$'),
               pos = 0,
               regex = XRegExp(
                   '(?<name>[\\w-]+)' +
                       '\\s*:\\s*' +
    ...
    while ((match = XRegExp.exec(str, regex, pos)) != null) {
      var value = match.value.replace(
          /^['"]|['"]$/g, '')  // strip quotes from end of strings
          ;
  ...
  /**
   * Wraps each line of the string into <code/> tag with given style applied to
   * it.
   *
   * @param {String} str   Input string.
  ...
  function wrapLinesWithCode(str, css) {
    ...
    // Replace two or more sequential spaces with &nbsp; leaving last space
    // untouched.
    str = str.replace(/ {2,}/g, function(m) {
      var spaces = '';

      for (var i = 0, l = m.length; i < l - 1; i++) spaces += sh.config.space;
  ...
  function unindent(str) {
    var lines = splitLines(fixInputString(str)), indents = new Array(),
        regex = /^\s*/, min = 1000;
  ...
  function getMatches(code, regexInfo) {
    ...
    var index = 0, match = null, matches = [],
  ...
  function processUrls(code) {
    ...
    return code.replace(sh.regexLib.url, function(m) {
      ...
      // <http://google.com> The problem is that they get transformed into
      // &lt;http://google.com&gt; Where as &gt; easily looks like part of the
      // URL string.
  ...
  function stripCData(original) {
    var left = '<![CDATA[', right = ']]>',
        ...
        copy = trim(original), changed = false, leftLength = left.length,
  ...
  function quickCodeHandler(e) {
    var target = e.target,
        ...
        textarea = document.createElement('textarea'), highlighter;
    ...
    // using \r instead of \r or \r\n makes this work equally well on IE, FF and
    // Webkit
    code = code.join('\r');
  ...
  sh.HtmlScript = function(scriptBrushName) {
    var brushClass = findBrush(scriptBrushName), scriptBrush,
        xmlBrush = new sh.brushes.Xml(), bracketsRegex = null, ref = this,
    ...
    function process(match, info) {
      var code = match.code, matches = [], regexList = scriptBrush.regexList,
          ...
          htmlScript = scriptBrush.htmlScript, result;
  ...
  sh.Highlighter.prototype = {
    ...
    removeNestedMatches: function(matches) {
      ...
      for (var i = 0, l = matches.length; i < l; i++) {
        ...
        for (var j = i + 1, l = matches.length; j < l && matches[i] !== null;
             j++) {
          var itemJ = matches[j];
    ...
    /**
     * Creates an array containing integer line numbers starting from the
     * 'first-line' param.
    ...
    getLineHtml: function(lineIndex, lineNumber, code) {
      var classes = [
        'line', 'number' + lineNumber, 'index' + lineIndex,
    ...
    getLineNumbersHtml: function(code, lineNumbers) {
      var html = '', count = splitLines(code).length,
    ...
    getCodeLinesHtml: function(html, lineNumbers) {
      ...
      var lines = splitLines(html),
          ...
          firstLine = parseInt(this.getParam('first-line')), html = '',
      ...
      for (var i = 0, l = lines.length; i < l; i++) {
        ...
        html += this.getLineHtml(
            i, lineNumber,
            (spaces != null ? '<code class="' + brushName + ' spaces">' +
                     spaces + '</code>' :
                              '') +
                line);
      }
    ...
    getMatchesHtml: function(code, matches) {
      ...
      for (var i = 0, l = matches.length; i < l; i++) {
        ...
        result +=
            wrapLinesWithCode(
                code.substr(pos, match.index - pos), matchBrushName + 'plain') +
      ...
      result +=
          wrapLinesWithCode(code.substr(pos), getBrushNameCss() + 'plain');
    ...
    getHtml: function(code) {
      var html = '', classes = ['syntaxhighlighter'], tabSize, matches,
          lineNumbers;
      ...
      code = trimFirstAndLastLines(code).replace(
          /\r/g, ' ')  // IE lets these buggers through
          ;
      ...
      html = '<div id="' + getHighlighterId(this.id) + '" class="' +
          classes.join(' ') + '">' +
          (this.getParam('toolbar') ? sh.toolbar.getHtml(this) : '') +
          '<table border="0" cellpadding="0" cellspacing="0">' +
          this.getTitleHtml(this.getParam('title')) + '<tbody>' +
          '<tr>' +
          (gutter ?
     ...
     *
     * Constructor isn't used for initialization so that nothing executes during
     * necessary `new SyntaxHighlighter.Highlighter()` call when setting up
     * brush inheritence.
     *
    ...
    init: function(params) {
      ...
      // process light mode
      if (this.getParam('light') == true) this.params.toolbar =
...
function Brush() {
  function process(match, regexInfo) {
    ...
    if (match.attributes != null) {
      ...
      while ((attributes = XRegExp.exec(code, regex, pos)) != null) {
        result.push(new constructor(
            attributes.name, match.index + attributes.index, 'color1'));
        result.push(new constructor(
            attributes.value,
    ...
    if (tag != null)
      result.push(new constructor(
          tag.name, match.index + tag[0].indexOf(tag.name), 'keyword'));
