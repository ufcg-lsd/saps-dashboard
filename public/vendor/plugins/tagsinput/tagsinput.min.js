...
!function(a) {
  ...
  b.prototype = {
    ...
    add: function(b, c) {
      var d = this;
      if (!(d.options.maxTags && d.itemsArray.length >= d.options.maxTags ||
            b !== !1 && !b)) {
        if ('string' == typeof b && d.options.trimValue && (b = a.trim(b)),
        ...
        if (!b.toString().match(/^\s*$/)) {
          ...
          var h = d.options.itemValue(b), i = d.options.itemText(b),
              j = d.options.tagClass(b), k = a.grep(d.itemsArray, function(a) {
                return d.options.itemValue(a) === h
              })[0];
          if (!k || d.options.allowDuplicates) {
            if (!(d.items().toString().length + b.length + 1 >
                  d.options.maxInputLength)) {
              var l = a.Event('beforeItemAdd', {item: b, cancel: !1});
              if (d.$element.trigger(l), !l.cancel) {
                d.itemsArray.push(b);
                var m =
                    a('<span class="tag ' + e(j) + '">' + e(i) +
                      '<span data-role="remove"></span></span>');
                if (m.data('item', b), d.findInputWrapper().before(m),
                    m.after(' '),
                    d.isSelect &&
                        !a('option[value="' + encodeURIComponent(h) + '"]',
                           d.$element)[0]) {
    ...
    refresh: function() {
      ...
      a('.tag', b.$container).each(function() {
        var c = a(this), d = c.data('item'), f = b.options.itemValue(d),
            g = b.options.itemText(d), h = b.options.tagClass(d);
        if (c.attr('class', null), c.addClass('tag ' + e(h)),
            c.contents()
                .filter(function() {
    ...
    build: function(b) {
      ...
      if (e.options = a.extend({}, h, b),
          ...
          c(e.options, 'itemValue'), c(e.options, 'itemText'),
          d(e.options, 'tagClass'), e.options.typeahead) {
        ...
        d(i, 'source'), e.$input.typeahead(a.extend({}, i, {
          source: function(b, c) {
            ...
            a.isFunction(g.success) ?
                g.success(d) :
                a.isFunction(g.then) ? g.then(d) : a.when(g).then(d)
          },
          updater: function(a) {
            e.add(this.map[a])
          },
      ...
      e.$container.on(
          ...
          a.proxy(
              ...
              e)),
          e.options.addOnBlur && e.options.freeInput &&
          e.$input.on(
              'focusout',
              a.proxy(
                  function() {
              ...
              a.proxy(
                  function(b) {
                    ...
                    if (e.$element.attr('disabled'))
                      return void e.$input.attr('disabled', 'disabled');
                    var d = c.val(),
                        f = e.options.maxChars &&
                        d.length >= e.options.maxChars;
                    e.options.freeInput && (g(b, e.options.confirmKeys) || f) &&
    ...
    destroy: function() {
      ...
      a.$container.off('keypress', 'input'),
          a.$container.off('click', '[role=remove]'), a.$container.remove(),
          a.$element.removeData('tagsinput'), a.$element.show()
    },
    focus: function() {
      this.$input.focus()
    },
    ...
    findInputWrapper: function() {
      for (var b = this.$input[0], c = this.$container[0];
           b && b.parentNode !== c;)
        b = b.parentNode;
      return a(b)
    }
  ...
  a.fn.tagsinput = function(c, d) {
    ...
    return this.each(function() {
      ...
      if (f)
        ...
        f = new b(this, c), a(this).data('tagsinput', f), e.push(f),
        'SELECT' === this.tagName &&
  ...
  var i = a('<div />');
  a(function() {
    a('input[data-role=tagsinput], select[multiple][data-role=tagsinput]')
        .tagsinput()
  })
}(window.jQuery);