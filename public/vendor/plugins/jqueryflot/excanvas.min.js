if (!document.createElement('canvas').getContext) {
  (function() {
    ...
    function ac(i) {
      return i.style + ' ' + i.variant + ' ' + i.weight + ' ' + i.size + 'px ' +
          i.family
    }
    ...
    q.drawImage = function(aq, m) {
      ...
      var ag = 10;
      ax.push(
          ' <g_vml_:group', ' coordsize="', d * i, ',', d * ag, '"',
          ' coordorigin="0,0"', ' style="width:', i, 'px;height:', ag,
      ...
      if (this.m_[0][0] != 1 || this.m_[0][1] || this.m_[1][1] != 1 ||
        ...
        ax.push(
            'padding:0 ', n(av.x / d), 'px ', n(av.y / d),
            'px 0;filter:progid:DXImageTransform.Microsoft.Matrix(', Z.join(''),
            ', sizingmethod=\'clip\');')
      } else {
        ax.push('top:', n(az.y / d), 'px;left:', n(az.x / d), 'px;')
      }
      ax.push(
          ' ">', '<g_vml_:image src="', aq.src, '"', ' style="width:', d * al,
          'px;', ' height:', d * ay, 'px"', ' cropleft="', ao / ai, '"',
          ' croptop="', am / aw, '"', ' cropright="', (ai - ao - at) / ai, '"',
          ' cropbottom="', (aw - am - aA) / aw, '"', ' />', '</g_vml_:group>');
    ...
    q.stroke = function(ao) {
      ...
      for (var aj = 0; aj < this.currentPath_.length; aj += ag) {
        ...
        var ah = false;
        am.push(
            '<g_vml_:shape', ' filled="', !!ao, '"',
            ' style="position:absolute;width:', Z, 'px;height:', ap, 'px;"',
            ' coordorigin="0,0"', ' coordsize="', d * Z, ',', d * ap, '"',
            ' stroked="', !ao, '"', ' path="');
        var aq = false;
        for (var ak = aj; ak < Math.min(aj + ag, this.currentPath_.length);
             ak++) {
          if (ak % ag == 0 && ak > 0) {
            am.push(
                ' m ', n(this.currentPath_[ak - 1].x), ',',
          ...
          switch (m.type) {
            ...
            case 'bezierCurveTo':
              am.push(
                  ' c ', n(m.cp1x), ',', n(m.cp1y), ',', n(m.cp2x), ',',
                  n(m.cp2y), ',', n(m.x), ',', n(m.y));
              break;
            case 'at':
            case 'wa':
              am.push(
                  ' ', m.type, ' ', n(m.x - this.arcScaleX_ * m.radius), ',',
                  n(m.y - this.arcScaleY_ * m.radius), ' ',
                  n(m.x + this.arcScaleX_ * m.radius), ',',
                  n(m.y + this.arcScaleY_ * m.radius), ' ', n(m.xStart), ',',
    ...
    function w(m, ag) {
      ...
      ag.push(
          '<g_vml_:stroke', ' opacity="', Z, '"', ' joinstyle="', m.lineJoin,
          '"', ' miterlimit="', m.miterLimit, '"', ' endcap="', S(m.lineCap),
          '"', ' weight="', i, 'px"', ' color="', p, '" />')
    }
    function G(aq, ai, aK, ar) {
      ...
      if (aj instanceof U) {
        ...
        ai.push(
            '<g_vml_:fill type="', aj.type_, '"', ' method="none" focus="100%"',
            ' color="', au, '"', ' color2="', at, '"', ' colors="',
            aE.join(','), '"', ' opacity="', ay, '"', ' g_o_:opacity2="', az,
            '"', ' angle="', an, '"', ' focusposition="', aF.x, ',', aF.y,
            '" />')
      } else {
        if (aj instanceof T) {
          if (j && p) {
            ...
            var aC = -aK.y;
            ai.push(
                '<g_vml_:fill', ' position="', ah / j * aB * aB, ',',
                aC / p * aA * aA, '"', ' type="tile"', ' src="', aj.src_,
                '" />')
          }
        } else {
    ...
    q.drawText_ = function(am, ak, aj, ap, ai) {
      ...
      ag.push(
          '<g_vml_:line from="', -j, ' 0" to="', ar, ' 0.05" ',
      ...
      ag.push(
          '<g_vml_:skew on="t" matrix="', an, '" ', ' offset="', al,
          '" origin="', j, ' 0" />', '<g_vml_:path textpathok="true" />',
          '<g_vml_:textpath on="true" string="', af(am),
          '" style="v-text-align:', Z, ';font:', af(p), '" /></g_vml_:line>');
