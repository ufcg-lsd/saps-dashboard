  ...
  validate: function(options) {
    ...
    if (validator.settings.onsubmit) {
      this.validateDelegate(':submit', 'click', function(event) {
        ...
        // allow suppressing validation by adding a cancel class to the submit
        // button
        ...
        function handle() {
          ...
          if (validator.settings.submitHandler) {
            ...
            validator.settings.submitHandler.call(
                validator, validator.currentForm, event);
            if (validator.submitButton) {
  ...
  rules: function(command, argument) {
    ...
    var data = $.validator.normalizeRules(
        $.extend(
            ...
            $.validator.attributeRules(element), $.validator.dataRules(element),
...
$.extend($.validator, {
      ...
      if (this.settings.focusCleanup && !this.blockFocusCleanup) {
        if (this.settings.unhighlight) {
          this.settings.unhighlight.call(
              this, element, this.settings.errorClass,
  ...
  prototype: {
        ...
        try {
          $(this.findLastActive() ||
            this.errorList.length && this.errorList[0].element || [])
              ...
              // isn't called, findLastActive won't have anything to find
              .trigger('focusin');
        } catch (e) {
    ...
    errors: function() {
      ...
      return $(
          this.settings.errorElement + '.' + errorClass, this.errorContext);
    },
    ...
    elementValue: function(element) {
      ...
      if (type === 'radio' || type === 'checkbox') {
        return $('input[name=\'' + $(element).attr('name') + '\']:checked')
            .val();
      }
    ...
    check: function(element) {
      ...
      for (var method in rules) {
        ...
        try {
          ...
          // valid, don't mark it as valid when there are no other rules
          ...
          if (this.settings.debug && window.console) {
            console.log(
                'Exception occurred when checking element ' + element.id +
    ...
    formatAndAdd: function(element, rule) {
      ...
      if (typeof message === 'function') {
        ...
        message = $.validator.format(
            message.replace(theregex, '{$1}'), rule.parameters);
      }
    ...
    defaultShowErrors: function() {
      ...
      for (i = 0; this.errorList[i]; i++) {
        ...
        if (this.settings.highlight) {
          this.settings.highlight.call(
              this, error.element, this.settings.errorClass,
              this.settings.validClass);
        }
      ...
      if (this.settings.unhighlight) {
        for (i = 0, elements = this.validElements(); elements[i]; i++) {
          this.settings.unhighlight.call(
              this, elements[i], this.settings.errorClass,
              this.settings.validClass);
        }
      }
  ...
  attributeRules: function(element) {
    ...
    for (var method in $.validator.methods) {
      ...
      // backwards compability allows type="date" and others to be compared as
      // strings
  ...
  methods: {
    ...
    equalTo: function(value, element, param) {
      // bind to the blur event of the target in order to revalidate whenever
...
// for easier event delegation handler is only called when
// $(event.target).is(delegate), in the scope of the jquery-object for
// event.target
(function($) {
$.extend($.fn, {
