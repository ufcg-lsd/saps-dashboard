...
$.ui.fancytree.registerExtension({
  ...
  treeInit: function(ctx) {
    var $tdFirst, $ul, tree = ctx.tree, $table = tree.widget.element;

    tree.tr = $('tbody tr', $table)[0];
    tree.columnCount = $('>td', tree.tr).length;
    // Perform default behavior
    this._super(ctx);
    // Standard Fancytree created a root <ul>. Now move this into first table
    // cell
    $ul = $(tree.rootNode.ul);
    ...
    $table
        .bind(
            'fancytreeactivate',
            function(event, data) {
              var i, tdList, node = data.node, tree = data.tree,
                             level = node.getLevel();
              ...
              // Expand nodes on activate, so we populate the right neighbor
              // cell
  ...
  nodeRender: function(ctx, force, deep, collapsed, _recursive) {
    ...
    var level, $tdChild, $ul, tree = ctx.tree, node = ctx.node,
                              $span = $(node.span);
