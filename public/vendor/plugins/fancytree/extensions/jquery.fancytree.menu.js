...
$.ui.fancytree.registerExtension({
  ...
  // `this._super`: the virtual function that was overridden (member of prev.
  // extension or Fancytree)
  treeInit: function(ctx) {
    var opts = ctx.options, tree = ctx.tree;

    this._super(ctx);

    // Prepare an object that will be passed with menu events
    tree.ext.menu.data = {tree: tree, node: null, $menu: null, menuId: null};
    ...
    tree.$container.delegate(
        'span.fancytree-node', 'contextmenu', function(event) {
          var node = $.ui.fancytree.getNode(event), ctx = {
            node: node,
            tree: node.tree,
            originalEvent: event,
    ...
    $(opts.menu.selector)
        .menu({
          ...
          focus: function(event, ui) {
            var data = $.extend(
                {}, tree.ext.menu.data,
                {menuItem: ui.item, menuId: ui.item.find('>a').attr('href')});
  ...
  _openMenu: function(ctx) {
    var data, tree = ctx.tree, opts = ctx.options,
    ...
    //        $menu.position($.extend({my: "left top", at: "left bottom", of:
    //        event}, opts.menu.position));
  ...
  _closeMenu: function(ctx) {
    var $menu, tree = ctx.tree, opts = ctx.options,
               data = $.extend({}, tree.ext.menu.data);
