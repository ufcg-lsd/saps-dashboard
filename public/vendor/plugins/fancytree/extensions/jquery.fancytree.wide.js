...
function realInnerWidth($el) {
  ...
  return $el[0].clientWidth -
      ($el.innerWidth() - parseFloat($el.css('width'), 10));
}


/**
 * [ext-wide] Recalculate the width of the selection bar after the tree
 * container was resized.<br> May be called explicitly on container resize,
 * since there is no resize event for DIV tags.
 *
 * @alias Fancytree#wideUpdate
 * @requires jquery.fancytree.wide.js
 */
$.ui.fancytree._FancytreeClass.prototype.wideUpdate = function() {
  var inst = this.ext.wide, prevCw = inst.contWidth, prevLo = inst.lineOfs;
  // http://blog.jquery.com/2012/08/16/jquery-1-8-box-sizing-width-csswidth-and-outerwidth/
  //	inst.contWidth = parseFloat(this.$container.css("width"), 10);
...
$.ui.fancytree.registerExtension({
  ...
  treeCreate: function(ctx) {
    ...
    this._super(ctx);
    // http://blog.jquery.com/2012/08/16/jquery-1-8-box-sizing-width-csswidth-and-outerwidth/
    //		this._local.contWidth = parseFloat(ctx.tree.$container.css("width"),
    //10);
    ...
    $(window).on(
        'resize' + ctx.widget.eventNamespace,
        $.ui.fancytree.debounce(200, function(event) {
  ...
  nodeRenderTitle: function(ctx) {
    var ofs, res, margin, instOpts = ctx.options.wide, inst = this._local,
                          cw = inst.contWidth, node = ctx.node;
