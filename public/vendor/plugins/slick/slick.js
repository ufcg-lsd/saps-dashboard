  ...
  if (_.transformsEnabled === false) {
    if (_.options.vertical === false) {
      _.$slideTrack.animate(
          {left: targetLeft}, _.options.speed, _.options.easing, callback);
    } else {
      _.$slideTrack.animate(
          {top: targetLeft}, _.options.speed, _.options.easing, callback);
    }
...
Slick.prototype.changeSlide = function(event, dontAnimate) {
  var _ = this, $target = $(event.target), indexOffset, slideOffset,
      unevenOffset, navigables, prevNavigable;
...
Slick.prototype.fadeSlide = function(oldSlide, slideIndex, callback) {
  ...
  if (_.cssTransitions === false) {
    ...
    _.$slides.eq(oldSlide).animate(
        {opacity: 0}, _.options.speed, _.options.easing);
    ...
    _.$slides.eq(oldSlide).css({opacity: 0});
...
Slick.prototype.getLeft = function(slideIndex) {
  var _ = this, targetLeft, verticalHeight, verticalOffset = 0, slideWidth,
      targetSlide;
...
Slick.prototype.initializeEvents = function() {
  ...
  _.$list.on(
      'touchstart.slick mousedown.slick', {action: 'start'}, _.swipeHandler);
  _.$list.on(
      'touchmove.slick mousemove.slick', {action: 'move'}, _.swipeHandler);
  _.$list.on('touchend.slick mouseup.slick', {action: 'end'}, _.swipeHandler);
  _.$list.on(
      'touchcancel.slick mouseleave.slick', {action: 'end'}, _.swipeHandler);
  ...
  if (_.options.centerMode === true) {
    ...
    if (_.options.infinite === true) {
      if (index >= centerOffset && index <= (_.slideCount - 1) - centerOffset) {
        ...
        allSlides
            .slice(
                indexOffset - centerOffset + 1, indexOffset + centerOffset + 2)
            .addClass('slick-active');
      }
...
Slick.prototype.slideHandler = function(index, sync, dontAnimate) {
  ...
  if (_.options.infinite === false && _.options.centerMode === false &&
    ...
    return;
      ...
      _.options.infinite === false && _.options.centerMode === true &&
...
Slick.prototype.swipeHandler = function(event) {
  ...
  if ((_.options.swipe === false) ||
    ...
    return;
      ...
      _.options.draggable === false && event.type.indexOf('mouse') !== -1) {
    return;
  }
...
Slick.prototype.swipeMove = function(event) {
  ...
  if (_.options.vertical === false) {
    ...
    _.swipeLeft = curLeft +
        (_.touchObject.swipeLength * (_.$list.height() / _.listWidth)) *
            positionOffset;
  }
...
Slick.prototype.unload = function() {
  ...
  _.$slides.removeClass('slick-slide slick-active slick-visible')
      .css('width', '');
};
...
$.fn.slickGoTo = function(slide, dontAnimate) {
  ...
  return _.each(function(index, element) {
    element.slick.changeSlide(
        {data: {message: 'index', index: parseInt(slide)}}, dontAnimate);
  });
};
